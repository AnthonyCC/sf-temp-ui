<%@ page import="com.freshdirect.webapp.taglib.coremetrics.CmRegistrationTag, java.net.*,
com.freshdirect.framework.util.NVL, com.freshdirect.webapp.taglib.fdstore.EnumUserInfoName,
com.freshdirect.fdstore.customer.FDUserI, com.freshdirect.webapp.taglib.fdstore.SessionName,
com.freshdirect.common.address.AddressModel, com.freshdirect.fdstore.referral.FDReferralManager,
java.util.Map, com.fasterxml.jackson.core.JsonGenerationException,
com.fasterxml.jackson.databind.JsonMappingException, com.fasterxml.jackson.databind.ObjectMapper,
com.freshdirect.webapp.ajax.registration.RegistrationRequest, com.freshdirect.framework.webapp.ActionError,
java.util.Collection, com.freshdirect.webapp.util.JspMethods" %><%@ taglib uri="freshdirect" prefix="fd" %><fd:CheckLoginStatus /><%
	FDUserI user = (FDUserI)session.getAttribute(SessionName.USER);

	/*
	 *	There are additional values not passed in, like "terms" which needs to be true
	 *	for the registration to succeed.
	 */

	//set all defaults, check against req params
	...
	String repeat_password = NVL.apply(request.getParameter(EnumUserInfoName.REPEAT_PASSWORD.getCode()), "");
	
	String passwordhint = NVL.apply(request.getParameter(EnumUserInfoName.PASSWORD_HINT.getCode()), "");
	
	String terms = NVL.apply(request.getParameter(EnumUserInfoName.CUSTOMER_AGREEMENT.getCode()), "true");
	String submission = NVL.apply(request.getParameter("submission"), "done");
	String actionName = NVL.apply(request.getParameter("actionName"), "ordermobilepref");
	
	
	String posn = "right";
	String isAjax = NVL.apply(request.getParameter("isAjax"), "false");

	//now check for ajax post, overriding defaults
	/* keep data[MAP] structure in case this goes to api eventually */
	ObjectMapper mapper = new ObjectMapper();
	RegistrationRequest jsonData = null;
	String reqJson = request.getParameter( "data" );
	//String reqJson = request.getParameterMap().toString();
	System.out.println("=========="+reqJson);
	
	if (reqJson != null) {
		jsonData = new RegistrationRequest();
		reqJson = URLDecoder.decode(reqJson, "UTF-8");
		jsonData = mapper.readValue(reqJson, RegistrationRequest.class);
	}
	
	if ("true".equalsIgnoreCase(jsonData.getIsAjax())) {
		successPage = jsonData.getSuccessPage();
		serviceType = jsonData.getServiceType();
		
		firstname = jsonData.getFirstName();
		lastname = jsonData.getLastName();
		zipcode = jsonData.getZipcode();
		email = jsonData.getEmail();
		repeat_email = jsonData.getEmailConfirm();
		
		password = jsonData.getPassword();
		repeat_password = jsonData.getPasswordConfirm();
		passwordhint = jsonData.getSecurityQuestion();
		
		terms = jsonData.getTerms();
		isAjax = jsonData.getIsAjax();		
	}
	...
	//System.out.println("=========isAjax is: "+isAjax);
	    
    CmRegistrationTag.setRegistrationLocation(session,"signup lite");
    
    String saContAction = ("true".equalsIgnoreCase(isAjax)) ? "signupLiteAjax": "signupLite";
%><fd:SiteAccessController action='<%= saContAction %>' successPage='<%= successPage %>' moreInfoPage='' failureHomePage='<%= failurePage %>' result='result' regRequest='<%= jsonData %>'><% 
if ("true".equalsIgnoreCase(isAjax)) { 
	...
	if (result.isFailure()) {
		///get error msgs
		Collection<ActionError> errors = result.getErrors();
		int i = errors.size();
		for (ActionError err : errors) {
			i--;
			errorMsgs += "\""+JspMethods.javaScriptEscape(err.getType())+"\": \""+JspMethods.javaScriptEscape(err.getDescription())+"\""+((i>0)?",":"");
		}
	}
	errorMsgs += "}";
...
%>{
	...
	//do normal sign up light overlay
%>
	<%
