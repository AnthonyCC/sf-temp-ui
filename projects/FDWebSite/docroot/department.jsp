...
<% try {
    ...
    String introCopy = introCopyAttribute==null?"":((Html)introCopyAttribute.getValue()).getPath(); //getContent("ATR_intro_copy").getString("url");
    String introTitle = department.getEditorialTitle();//geteContent("ATR_intro_copy").getString("ATR_editorial_title");
...
//    if ( introTitle==null || introTitle.length() < 1) {
//        introTitle = department.getFullName();
//    }
...
/*
    if (department.getBoolean("ATR_contains_organic",false)==true) {
     	organicLegend.append("<IMG SRC=\"");
        organicLegend.append(organicImage);
	organicLegend.append("\" BORDER=\"0\" ALT=\"Organic Available\" HSPACE=\"0\" VSPACE=\"0\" ");
	organicLegend.append("height=\"13\" width=\"104\">");
    } */

// temporary: if grocery, wine or bakery department..then we dont want to use the stantard logic
if ( "OUR_PICKS, FRO, GRO, DAI, SPE".indexOf(department.getContentName().toUpperCase())==-1  && !"bak".equals(request.getParameter("deptId")) && !"win".equals(request.getParameter("deptId")) && introCopy!=null  && introCopy.trim().length()>0) {%>
...
<%if(introTitle!=null || !"".equals(introTitle)){%><%=introTitle%><%}%></font><BR><IMG src="/media_stat/images/layout/clear.gif" WIDTH="1" HEIGHT="3"><BR><fd:IncludeMedia name='<%= introCopy %>' /><BR><IMG src="/media_stat/images/layout/clear.gif" WIDTH="1" HEIGHT="4"></td>
...
<table width="550" CELLPADDING="0" CELLSPACING="0" BORDER="0">
	...
	<tr valign="top">
		<td width="335"><font class="text10bold">Compare <%= department.getFullName() %> by:&nbsp;</font><%=rateNRankLinks.toString()%><%-- include file="/include/i_show_rating_groups.jsp" --%>
...
<%}
 if (true==false) { //( "our_picks".equals(request.getParameter("deptId")) && introCopy!=null  && introCopy.trim().length()>0) {
%><table width="550" CELLPADDING="0" CELLSPACING="0" BORDER="0">
<tr valign="top"><td class="text11" width="550">
...
<%} else {%><IMG src="/media_stat/images/layout/clear.gif" WIDTH="1" HEIGHT="10">
