...
<%
    ...
    if ( EnumServiceType.WEB.getName().equalsIgnoreCase(serviceType) && (isGiftCardEnabled || isRobinHoodEnabled) ) {

		String successPageFinal="";

		if (isRobinHoodEnabled){
			if (successPage.indexOf(rhLanding)>-1) {
				//success page has the landing page in it
				if (successPage.equalsIgnoreCase(rhLanding)) {
					//success page is the RH landing page
					successPageFinal = rhLanding;
				}
				//otherwise, successPage is what we want
				successPageFinal = successPage;
			}
		}

		if (isGiftCardEnabled){
			if (successPage.indexOf(gcLanding)>-1) {
				//success page has the landing page in it
				if (successPage.equalsIgnoreCase(gcLanding)) {
					//success page is the RH landing page
		...
		//System.out.println("successPageFINAL "+successPageFinal);
