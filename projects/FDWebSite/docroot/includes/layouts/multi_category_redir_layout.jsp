     ...
     if (contentNode instanceof CategoryModel){
        ...
        if (cat.getParentNode()!=null && cat.getParentNode().getPK().getId().equals(currentFolderPKId)) {
            ...
            if (newCategoryCount > 0) {  // print the seperator bar if one or more categories has been printed  
				if (textRow.length() > 1) { %><tr valign="top"><%=textRow.toString()%></tr> 
<%  			}
    			textRow.setLength(0); %>
               </tr></table>
...
<% /*            <tr><td><img src="/media_stat/images/layout/clear.gif" width="1" height="5"></td></tr>
				<tr><td BACKGROUND="/media_stat/images/layout/summer_line.gif" width="< %=maxWidth -% >">&nbsp;</td></tr> */  %>
...
<%          } %>
            ...
            <table CELLPADDING="6" CELLSPACING="0" BORDER="0"><tr align="center" valign="top">
<%
		  openedTable=true;	  
          continue; 
        } else {
...
%>
    <td valign="bottom" align="center" WIDTH="<%=displayObj.getImageWidth() %>">
        <a href="<%=displayObj.getItemURL()%>"><img src="<%= displayObj.getImagePath()%>"  <%=displayObj.getImageDimensions() %> ALT="<%=displayObj.getAltText()%>" hspace="0" border="0"></a></td>
<% 
