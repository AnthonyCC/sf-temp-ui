...
int cellWidth = 136;  // default cell width
int tblWidth = 0;
//**********************  End of stuff to make JSPF's become JSP's
//**************************************************************
//***          Vertical Layout Pattern                       ***
//**************************************************************
final int PRODUCT_CELL_WIDTH=136;
...
<fd:VerticalPattern id='itemsToDisplay' showFolder='true' returnCategory='true' columns='<%= MAX_COLUMNS %>' collection='<%=availableList%>' >
<%

//    boolean showSeparator[] = new boolean[itemsToDisplay.length-1];
	if (itemsToDisplay.length>0) {
        tblWidth = (PRODUCT_CELL_WIDTH * itemsToDisplay.length) + (MAX_COLUMNS - 1) ;
        // determine, one time, where the separators should be displayed.
        if (firstRow) {
			currRow = 1;
            firstRow = false;  
...
Image headerLabel = null;
	...
	for (int nodeCounter=0; nodeCounter<itemsToDisplay.length; nodeCounter++) {
                ...
                if (currentNode!=null) {
					...
					if (currentNode.hasAttribute("CAT_LABEL")) {
						hasHeaderLabel = true;
						headerLabel = (Image)currentNode.getAttribute("CAT_LABEL").getValue();
					}
					
					if (currentNode instanceof CategoryModel && hasHeaderLabel) {
						if (((CategoryModel)currentNode).getDepartment().equals(currentNode.getParentNode())) {
...
<td width="<%= PRODUCT_CELL_WIDTH %>"><% if (isParentCategory && headerLabel != null && !"".equals(headerLabel.getPath())) { %><a href="<%=displayObj.getItemURL()%>&trk=<%=pageTrkCode%>" <%=displayObj.getRolloverString()%> style="font-weight: bold;"><img src="<%=headerLabel.getPath()%>" width="<%=headerLabel.getWidth()%>" height="<%=headerLabel.getHeight()%>" border="0"></a><br><% } else { %><div style="width: <%=displayObj.getImageWidth()%>px; cursor: hand;" ><a href="<%=displayObj.getItemURL()%>&trk=<%=pageTrkCode%>" <%=displayObj.getRolloverString()%>><img src="<%=displayObj.getImagePath()%>"  name="<%=displayObj.getImageName()%>" width="<%=displayObj.getImageWidth()%>"  height="<%=displayObj.getImageHeight()%>" ALT="<%=displayObj.getAltText()%>" border="0" vspace="3"></a><br><a href="<%=displayObj.getItemURL()%>&trk=<%=pageTrkCode%>"><span class="<%=itemNameFont%>"><%=displayObj.getItemName()%></span></a></div><br><font class="space4pix"><br></font><% } %><img src="/media_stat/images/layout/clear.gif" width="<%= PRODUCT_CELL_WIDTH %>" height="1" ><br>              
</td>
<%
                } else {
%>
<td width="<%= PRODUCT_CELL_WIDTH %>"><img src="/media_stat/images/layout/clear.gif" width="1" height="1"  ALT="" border="0" ><font class="space4pix"><br></font>              
</td>
<%                }
