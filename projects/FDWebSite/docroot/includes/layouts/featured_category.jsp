...
// display objects
DisplayObjectRepo repo = new DisplayObjectRepo(response,prodNameAttribute,trkCode);
...
for(Iterator layoutI = Arrays.asList(new Object[] {featuredTableLayout, nonFeaturedTableLayout}).iterator(); layoutI.hasNext(); ) {
	...
	for(int i = 0; i< rows.size(); ++i) {
		...
		if (rowWidth == 0) rowWidth = maxWidth;
%>
...
%>
		<td align="center" width="<%=displayObject.getImageWidth()%>">
			<a href="<%=displayObject.getItemURL()%>&trk=<%=pageTrkCode%>"><img 
				src="<%=displayObject.getImagePath()%>"
				<%=displayObject.getImageDimensions()%>
				alt="<%=displayObject.getAltText()%>"
				hspace="0" border="0"/></a>
		</td>
...
<%
		for(int j=0; j< row.size(); ++j) { // for each link
			ContentNodeModel contentNode = (ContentNodeModel)layout.getElement(i,j);
			if (separatorFilter.evaluate(contentNode)) break;
			DisplayObject displayObject = (DisplayObject)repo.getDisplayObject(contentNode);
			String fontClass = contentNode instanceof ProductModel ? "catPageProdNameUnderImg" : "text11";

%>
		<td align="center" width="<%=displayObject.getImageWidth()%>">
			<a href="<%=displayObject.getItemURL()%>&trk=<%=pageTrkCode%>"><font class="<%=fontClass%>">
                        <%= "fdi".equals(deptId) ? "Click here" : displayObject.getItemName()%>
			</font></a>
		</td>
<%
		} // links
%>
	...
	<tr><td colspan="<%= row.size() %>"><img src="/media_stat/images/layout/clear.gif" width="1" height="16"/></td></tr>
	</table>
<%
	}
} // layouts
