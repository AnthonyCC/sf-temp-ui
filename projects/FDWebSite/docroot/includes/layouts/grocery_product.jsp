...
if (brandValue==null) {
        ...
        BrandModel bm = (BrandModel) ContentFactory.getInstance().getContentNodeByName(brandValue);
        if (bm!=null){
...
<%
        ...
        if (brandsForProd!=null && brandsForProd.size()>0) {
                ...
                //Attribute brandAttrib = thisBrandModel.getAttribute("BRAND_LOGO_SMALL");
                Attribute brandLogoAttrib = thisBrandModel.getAttribute("BRAND_LOGO_SMALL");
                Attribute brandAttrib = thisBrandModel.getAttribute("BRAND_POPUP_CONTENT");
                if (brandAttrib!=null) {
                        TitledMedia tm = (TitledMedia)brandAttrib.getValue();
                        EnumPopupType popupType=EnumPopupType.getPopupType(tm.getPopupSize());
                ...
                if (brandLogoAttrib!=null){
                titleBrandLogo = (Image)brandLogoAttrib.getValue();
                }
...
<font class="title14" <% if (!skuAvailable) { %>color="#999999"<% } %>><%
        ...
        MultiAttribute varMatrix = (MultiAttribute)minSku.getAttribute("VARIATION_MATRIX");
