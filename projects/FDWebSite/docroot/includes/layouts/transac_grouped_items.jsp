...
<fd:FDShoppingCart id='cart' action='addMultipleToCart' result='result' successPage='<%= succPage %>'>
    ...
    %>
    <td width="30" align="center" colspan="2"><NOBR>
<%
...
%>
    ...
    %>
        <div class="qtyinput qtyinput_fixedwidth">
          <a href="javascript:chgQty(<%=itemShownIndex%>,'<%= qtyFldName %>', -<%= displayProduct.getQuantityIncrement() %>, <%= displayProduct.getQuantityMinimum() %>, <%= user.getQuantityMaximum(displayProduct) %>);return false;" class="quantity_minus"><span>Increase quantity</span></a>
          ...
          <a href="javascript:chgQty(<%=itemShownIndex%>,'<%= qtyFldName %>', <%= displayProduct.getQuantityIncrement() %>, <%= displayProduct.getQuantityMinimum() %>, <%= user.getQuantityMaximum(displayProduct) %>);return false;" class="quantity_plus"><span>Decrease quantity</span></a>
        </div>
    <%  }   %>
    ...
    <%  } else {    %>
    <td width="55" align="right">&nbsp;<font class="groceryProductLinePrice"><%= JspMethods.formatPrice(productInfo, user.getPricingContext())%></font>
    </td>
