...
<%
	...
	String noErrImage = ""; 
	String chefPickIndicator = "<img src=\"/media_stat/images/layout/star11.gif\" border=\"0\" alt=\"\" />";
	...
	String suffix = CartName.MODIFY_CART.equals(cartMode) || CartName.QUICKSHOP.equals(cartMode) || CartName.MODIFY_LIST.equals(cartMode) || CartName.ACCEPT_ALTERNATIVE.equals(cartMode) ? "" : "_0";
	...
	prodPopup = "/cg_meal_item_detail.jsp?mcatId="+parentCat+"&mproductId="+productNode+"&mskuCode="+defaultSku+"&";
	 ...
	 * shared vars
	 */
	FDProduct alcFdProd = null;
	SkuModel alcSku = null;
	boolean isAlc = false;
	String alcRest = "";
%>
...
<table width="<%=maxWidth%>" cellpadding="0" cellspacing="0" border="0" align="center">
	<tr>
		<td valign="top" align="center" colspan="3">
			...
			<span class="title16"><%= JspMethods.formatPrice(defaultProductInfo.getZonePriceInfo(user.getPricingContext().getZoneId()).getDefaultPrice()) %>&nbsp;-&nbsp;<%=productNode.getBlurb()%></span><br />
			<span class="space8pix"><br /></span>
		</td>
	</tr>
	<tr>
		<td valign="top"><%	if (imgMedia!=null) { %><img src="<%=imgMedia.getPath()%>" border="0" width="<%=imgMedia.getWidth()%>" height="<%=imgMedia.getHeight()%>" alt="" /><% } %></td>
...
<form id="productForm" name="productForm" method="POST" action="<%=formAction%>">
	...
	<%
		...
		alcRest = "";
		if (alcFdProd != null) {
			isAlc = alcFdProd.isAlcohol();
			alcRest = alcFdProd.getMaterial().getAlcoholicContent().getCode();
		}
	%>
		window.alcohol['<%=alcSku.getSkuCode()%>'] = { isAlc: <%=isAlc%>, alcRest: '<%=alcRest%>' }
	</script>

	<input type="hidden" name="isAlc<%= suffix %>" value="<%= isAlc %>" />
		...
		if (isAvailable) {
			...
			for (Iterator cgItr = componentGroups.iterator(); cgItr.hasNext(); compGrpIdx++) {
				...
				if (matCharNames.size() > 0) {
					...
					<table  width="<%=maxWidth%>" cellpadding="0" cellspacing="0" border="0" align="center">
						<tr>
							<td><img src="/media_stat/images/layout/clear.gif" width="95" height="6" alt="" /></td>
							<td><img src="/media_stat/images/layout/clear.gif" width="15" height="1" alt="" /></td>
							<td><img src="/media_stat/images/layout/clear.gif" width="<%=maxWidth-115%>" height="1" alt="" /></td>
						</tr>
						<tr>
						<% if (paintDropdownVertical) { %>
							...
							<td valign="top"><img src="/media_stat/images/layout/clear.gif" width="5" height="1" alt="" /></td>
							<td valign="top">
						<% } else { %>
						...
						if (showOptions) {
							...
							for (int prodIdx = 0; prodIdx < numProducts && showOptions ; prodIdx++) {
								...
								if (isChefsPick) {
									...
									%><img src="/media_stat/images/layout/clear.gif" width="14" height="11" alt="" />&nbsp;<%
								}

								%><a href="javascript:popup('<%=prodPopup%>catId=<%=oneProd.getParentNode()%>&prodId=<%=oneProd%>&skuCode=<%=oneProd.getDefaultSku()%>','<%=prodPopUpSize%>')" <%=imgRollOver%>><%=JspMethods.getDisplayName(oneProd,prodNameAttribute)%></a><br />
							<% }
						...
						for (Iterator mcnItr = matCharNames.iterator(); mcnItr.hasNext();) {
							...
							if ( varOpts!=null && varOpts.length>1) {
								...
								<select name="<%=mcName+suffix%>" class="text11" <%=(!paintDropdownVertical ?"style=\"width: 190px\"" :"")%>>
									<option value=""><%=(String)fdVarOptDesc.get(mcName)%></option>
									<%
										...
										for (int voIdx = 0; voIdx < varOpts.length; voIdx++) {
											...
											String selected= (varOpts[voIdx].getName().equals(templateLine.getOptions().get(mcName)) )
												...
												: varOpts[voIdx].getName().equals(request.getParameter(mcName+suffix))
													...
													: "";
											%>
											<option value="<%=varOpts[voIdx].getName()%>" <%=selected%>>&nbsp;<%=varOpts[voIdx].getDescription()%></option><!-- <%= optSkuCode %> --><%
										}
									%>
								</select>
								...
								<% if (paintDropdownVertical || selectTagCnt %2!=0  || !mcnItr.hasNext() ) { %>
									...
									%>
									<img src="/media_stat/images/layout/clear.gif" width="2" height="1" alt="" />
								<% }
				...
				<% } else {
					...
					if (!cartMode.equals(CartName.MODIFY_CART) && !CartName.MODIFY_LIST.equals(cartMode) && !CartName.ACCEPT_ALTERNATIVE.equals(cartMode) && !cartMode.equals(CartName.QUICKSHOP) && 		 optionalProducts.size()>0 ) {
						...
						%><logic:iterate id='optProd' collection="<%=optionalProducts%>" type="com.freshdirect.fdstore.content.ProductModel" indexId='optIdx'><%
							...
							if (!paintOptionalVertical) { // paint the optional category using the Horizontal style
								if (prodCount==0) { %><tr><% } %>
								<td valign="top" align="center" width="140"><img src="/media_stat/images/layout/clear.gif" width="140" height="1" alt="" /><br />
									<table width="140" border="0" cellspacing="0" cellpadding="0" align="center">
										...
										<tr>
											<td colspan="2" align="center">
												<a href="javascript:popup('prod_desc_popup.jsp?catId=<%=optProd.getParentNode()%>&prodId=<%=optProd%>','small')"><%=dispObj.getItemName()%></a><br /><%=dispObj.getPrice()%><br />
												...
												<table border="0" cellspacing="0" cellpadding="0" align="center">
													<tr>
														<td valign="bottom" align="right">
															...
															<input type="hidden" name="isAlc_<%= prodCount %>" value="<%= isAlc %>" />
															...
															<input name ="quantity_<%=prodCount%>" value="<%=(request.getParameter("quantity_"+prodCount)==null ?"" : request.getParameter("quantity_"+prodCount))%>" type="text" size="3" onChange="chgNamedQty(pricings[<%=prodCount%>],'quantity_<%=prodCount%>',0,<%= optProd.getQuantityMinimum() %>,<%= user.getQuantityMaximum(optProd) %>,true)">
														</td>
														<td valign="bottom" align="left">
															<a href="javascript:chgNamedQty(pricings[<%=prodCount%>],'quantity_<%=prodCount%>',<%= optProd.getQuantityIncrement() %>,<%= optProd.getQuantityMinimum() %>,<%= user.getQuantityMaximum(optProd) %>,true);"><img src="/media_stat/images/layout/grn_arrow_up.gif" width="10" height="9" border="0" vspace="1" alt="Increase quantity" /></a><br />
															<a href="javascript:chgNamedQty(pricings[<%=prodCount%>],'quantity_<%=prodCount%>',-<%= optProd.getQuantityIncrement() %>,<%= optProd.getQuantityMinimum() %>,<%= user.getQuantityMaximum(optProd) %>,true);"><img src="/media_stat/images/layout/grn_arrow_down.gif" width="10" height="9" border="0" vspace="1" alt="Decrease quantity" /></a>
															<img src="/media_stat/images/layout/clear.gif" width="10" height="1" alt="" />
														</td>
													</tr>
												</table>
											</td>
								...
								if (openTable) { // first item, paint main table 
									...
									%>
									<tr valign="top">
										<td width="115"><img src="<%=dispObj.getImagePath()%>" border="0" name="<%=imgName%>" alt="" /></td>
								...
								<tr valign="top">
									...
									<td width="<%=maxWidth-120%>">
										<a href="javascript:popup('prod_desc_popup.jsp?catId=<%=optProd.getParentNode()%>&prodId=<%=optProd%>','small')" <%=imgRollOver%>><%=dispObj.getItemNameWithoutBreaks()%></a><nobr> <%=(!"".equals(dispObj.getSalesUnitDescription()) ? dispObj.getSalesUnitDescription()+" - " : "")%><%=dispObj.getPrice()%></nobr>
										<br /><img src="/media_stat/images/layout/clear.gif" width="1" height="1" alt="" />
									</td>
								</tr>
			...
			if (prodCount > 0) { %>
				<table width="<%=maxWidth%>" cellpadding="0" cellspacing="0" border="0" align="center">
					<tr>
						<td>
							<br />
							<img src="/media_stat/images/layout/cccccc.gif" width="<%=maxWidth%>" height="1" border="0" alt="" /><br />
							<br />
						</td>
					</tr>
				</table>
				...
				if (productNode.hasTerms()) { %>
					<table width="<%=maxWidth%>" cellpadding="0" cellspacing="0" border="0" align="center">
						<tr>
							<td align="left"><fd:IncludeMedia name='<%= productNode.getProductTerms().getPath()%>' /></td>
						</tr>
						<tr>
							<td align="left">
						...
						<tr>
							<td align="left"><%= result.hasError("agreeToTerms") ? errImage : noErrImage%>
								<input type="checkbox" <%=(cartMode.equals(CartName.MODIFY_CART) || "yes".equalsIgnoreCase(request.getParameter("agreeToTerms")) ? "checked" : "")%> name="agreeToTerms" value="yes" />&nbsp;<b>I have read and agree to the terms above.</b><br />
								<br />
							</td>
						</tr>
					</table>
						...
						<td width="<%=maxWidth-257%>" align="right">
							...
							<% } else if (CartName.MODIFY_LIST.equals(cartMode) || CartName.ACCEPT_ALTERNATIVE.equals(cartMode)) {
								...
								if (CartName.ACCEPT_ALTERNATIVE.equals(cartMode)) {
									%>
									<input type="image" name="save_changes" src="/media_stat/images/template/quickshop/alt_replace_item_btn.gif" onclick="document.productForm.list_action.value='modify';document.productForm.submit();" hspace="2" border="0" /><br />
									<font class="space4pix"><br /></font>
									<%
								} else {
									%>
									<input type="image" name="save_changes" src="/media_stat/images/buttons/list_save_changes.gif" onclick="document.productForm.list_action.value='modify';document.productForm.submit();" hspace="2" border="0" /><br />
									<font class="space4pix"><br /></font>

									<input type="image" name="remove_from_list" src="/media_stat/images/buttons/remove_item.gif" hspace="2" border="0" onclick="document.productForm.list_action.value='remove';document.productForm.submit();" /><br />
									<font class="space4pix"><br /></font>
									<%
								} // cartMode
								...
								%>
								<a href="<%=referer%>"><img name="no_changes" src="/media_stat/images/buttons/no_change.gif" hspace="2" border="0"></a><br />
								<font class="space4pix"><br /></font>
							<% } else { %>
					...
					if (!CartName.MODIFY_CART.equals(cartMode) && !CartName.MODIFY_LIST.equals(cartMode)) {
						...
						<fd:CCLCheck>
							<td colspan="9" align="right">
								<br/>
								<div>
									<a id="ccl-add-action" href="/unsupported.jsp" onclick="return CCL.save_items('productForm',this,'action=CCL:AddMultipleToList&source=ccl_actual_selection')"><img src="/media_stat/ccl/lists_link_selected_dfgs.gif" width="133" height="13" style="border: 0; padding-left: 14px" alt="" /><img src="/media_stat/ccl/lists_save_icon_lg.gif" width="12" height="14" style="margin: 0 0 1px 5px; border: 0" alt="" /></a>
				...
				<script language="javascript">
					var pricings = [];
					<%
					for (int pidx=0; pidx < prodCount; pidx++) { 
						%>
						pricings[<%=pidx%>] = makePricing(<%=pidx%>, document.productForm['skuCode_<%=pidx%>'].value, document.productForm['salesUnit_<%=pidx%>'].value, document.productForm['quantity_<%=pidx%>'].value);
						<%
					}
					%>
					...
					function updatePriceField() {
						...
						for (var x=0; x < pricings.length; x++) {
							totPrice+= pricings[x].price;
						}
						
						document.productForm.price.value = totPrice==0 ? "" : "$" + currencyFormat(totPrice);
					}

					function makePricing(idx, skuCode, su, qty) {
						var po = new Pricing();
						
						if (idx === 0) {
							po.setSKU('<%=defaultSku.getSkuCode()%>');
							po.setSalesUnit('<%=defaultProduct.getSalesUnits()[0].getName()%>');
							po.setCallbackFunction( updatePriceField );
							po.setQuantity(<%=defaultQuantity%>);
						}else{
							po.setSKU(skuCode);
							po.setSalesUnit(su);
							po.setCallbackFunction(updatePriceField);
			...
			<table width="<%=maxWidth%>" cellpadding="0" cellspacing="0" border="0" align="center">
				<tr>
					<td width="100%">
						<div align="center">
							<font class="text12" color="#999999"><b>We're sorry! This item is temporarily unavailable.</b></font>
						</div>
					</td>
				</tr>
			</table>
					...
					<logic:iterate id='mediaRef' indexId='indexNo' collection="<%=catBottomMedias%>" type="com.freshdirect.fdstore.content.MediaModel">
						<% if (((Html)mediaRef).getPath()!=null) {
							if (indexNo.intValue()!=0){
								...
								<img src="/media_stat/images/layout/cccccc.gif" width="295" height="1" border="0" alt="" /><br />
								<img src="/media_stat/images/layout/clear.gif" width="1" height="10" alt="" border="0" alt="" /><br />
								<%
							}
							%><fd:IncludeMedia name='<%= ((Html)mediaRef).getPath() %>' /><%
						} %>
