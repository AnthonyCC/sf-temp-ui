			...
			<tr valign="top">
				<td colspan="2"><input type="hidden" name="salesUnit_<%= prodCount %>" value="<%= salesUnitName %>">
				  ...
				  <input type="hidden" name="productId_<%= prodCount %>" value="<%= optProd %>">
					...
					<!-- /fd:IsAlcoholic -->
          <div class="qtyinput">
            <a href="javascript:chgNamedQty(pricing_<%=prodCount%>,'quantity_<%=prodCount%>',-<%= optProd.getQuantityIncrement() %>,<%= optProd.getQuantityMinimum() %>,<%= user.getQuantityMaximum(optProd) %>,true);" class="quantity_minus"><span>Decrease quantity</span></a>
            ...
            <a href="javascript:chgNamedQty(pricing_<%=prodCount%>,'quantity_<%=prodCount%>',<%= optProd.getQuantityIncrement() %>,<%= optProd.getQuantityMinimum() %>,<%= user.getQuantityMaximum(optProd) %>,true);" class="quantity_plus"><span>Increase quantity</span></a>
          </div>
        </td>
...
<% }%>
	...
	<table width="<%=W_COMPONENT_GROUP_MEAL_TOTAL%>" cellpadding="0" cellspacing="0" border="0" align="center">
        ...
        <div class="qtyinput">
          ...
          <a href="javascript:chgNamedQty(pricing,'quantity<%=suffix%>',-<%=productNode.getQuantityIncrement() %>,<%= productNode.getQuantityMinimum() %>,<%= user.getQuantityMaximum(productNode) %>);" class="quantity_minus"><span>Decrease quantity</span></a>
          ...
          <a href="javascript:chgNamedQty(pricing,'quantity<%=suffix%>',<%= productNode.getQuantityIncrement() %>,<%= productNode.getQuantityMinimum() %>,<%= user.getQuantityMaximum(productNode) %>);" class="quantity_plus"><span>Increase quantity</span></a>
