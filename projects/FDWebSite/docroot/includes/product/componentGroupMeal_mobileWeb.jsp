<%-- // 
	...
	THIS LAYOUT IS NOT READY TO BE USED.
	
	IF IT'S NEEDED, MORE WORK MUST BE DONE ON IT.
	...
	if (pgErrs.size()>0) {
		...
		if(request.getAttribute("newLeftNav")==null) { %>
			<table width="<%=W_COMPONENT_GROUP_MEAL_TOTAL%>" cellpadding="0" cellspacing="0" border="0" align="center">
				<tr><td align="left"><%@ include file="/includes/i_error_messages.jspf" %></td></tr>
		 	</table>
		<% } else { %>
...
<%-- start mobweb display of product, manual gen, based on pdp --%>
	<div itemscope itemtype="http://schema.org/Product" class="pdp">
		<div class="prodDetail">
			<div class="componentGroupMeal">
				<div class="prodDetail-images">
					<% if (imgMedia!=null) { %>
						<div class="main-image">
							<img itemprop="image" src="<%=imgMedia.getPath()%>" alt="Product Image - <%=productNode.getFullName()%>" data-thumbnails-main="" data-large-url="">
							<div class="hd" style="display:none" data-thumbnails-hd=""></div>
						</div>
					<% } %>
				</div>
				<div class="prodDetail-nrp"><%-- name/ratings/prices --%>
					<h1 class="pdpTitle" style="display:inline">
						<span itemprop="name"><%=productNode.getFullName()%></span>
						<% if (prodSubtitle!=null && !"".equals(prodSubtitle)) { %>
							<span class="title13"><%=productNode.getSubtitle()%></span><br>
						<% } %>  
						<span class="annotation cssbutton transparent white icon-info2-before notext">?</span>
					</h1>
					<div class="pdp-price"><%= JspMethods.formatPrice(defaultProductInfo.getZonePriceInfo(user.getPricingContext().getZoneInfo()).getDefaultPrice()) %></div>
					
					<div class="span-7 prepend-1 first pdp-info"><%=productNode.getBlurb()%></div>
				</div>
			</div>
		</div>
	</div>
	<soy:render template="common.skuControlVariations" data="${productPotato}" />
	skuControlVariations
	
	<div class="DEBUG">HERE DOWN IS OLD DESIGN</div>
	
	
<%-- start display of product --%>
	<table class="DEBUG" width="<%=W_COMPONENT_GROUP_MEAL_TOTAL%>" cellpadding="0" cellspacing="0" border="0" align="center">
		<tr><td valign="top" align="center" colspan="3">
			...
			<span class="title16"><%= JspMethods.formatPrice(defaultProductInfo.getZonePriceInfo(user.getPricingContext().getZoneInfo()).getDefaultPrice()) %>&nbsp;-&nbsp;<%=productNode.getBlurb()%></span><br>
		...
		if (matCharNames.size() > 0) {
			...
			<table  width="<%=W_COMPONENT_GROUP_MEAL_TOTAL%>" cellpadding="0" cellspacing="0" border="0" align="center">
			  <tr>
				...
				<td><img src="/media_stat/images/layout/clear.gif" alt="" width="<%=W_COMPONENT_GROUP_MEAL_TOTAL-115%>" height="1"></td>
			  </tr>
   			  <tr>
...
<%      }
	...
	<table width="<%=W_COMPONENT_GROUP_MEAL_TOTAL%>" cellpadding="0" cellspacing="0" border="0" align="center">
		<tr><td><BR>
            <IMG src="/media_stat/images/layout/cccccc.gif" ALT="" WIDTH="<%=W_COMPONENT_GROUP_MEAL_TOTAL%>" HEIGHT="1" BORDER="0"><BR><br>
        </td></tr>
    </table>
...
<fd:CCLCheck>
	...
	<div>
	    <a id="ccl-add-action" href="/unsupported.jsp" 
		      onclick="return CCL.save_items('productForm',this,'action=CCL:AddMultipleToList&source=ccl_actual_selection')"><img src="/media_stat/ccl/lists_link_selected_dfgs.gif" alt="save to lists" width="133" height="13" style="border: 0; padding-left: 14px"><img src="/media_stat/ccl/lists_save_icon_lg.gif" alt="save to lists" width="12" height="14" style="margin: 0 0 1px 5px; border: 0"/></a>
