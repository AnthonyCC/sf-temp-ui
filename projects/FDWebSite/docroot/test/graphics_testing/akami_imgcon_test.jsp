...
<head>
	...
	<style>
		...
		.akaTitle {
			...
			background: #cccccc url("https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.3/themes/smoothness/images/ui-bg_highlight-soft_75_cccccc_1x100.png") 50% 50% repeat-x;
			color: #222222;
			font-weight: bold;
			line-height: 2em;
		}
		...
		.slider-opacity, .slider-quality { margin: 0 5px 0 5px; }
		.slider-opacity-value, .slider-quality-value { font-weight: bold; color: #fc0; }
		...
		.alignC { text-align: left !important; }
		.fleft { float: left; }
		.fright { float: right; }
		.clrL { clear: left; }
		.clrB { clear: both; }
...
<div id="akaCont">
	<div class="akaTitle">
		Akami Image Convertor
		<span class="ajaxImageSizes inBlock fright"><input id="ajaxImageSizes" type="checkbox" /><label for="ajaxImageSizes">Ajax Image Sizes</label></span>
	</div>
	<div id="akaForm">
		<div id="akaCmdAccordion" class="accordion fleft">
			<h3>Measurement <span class="ui-icon ui-icon-help akaCmdGroupHelp helpToolTip" title="The definitions you create with the unit command affect only the commands that follow it. Therefore, it is recommended that the unit command be the first command (if used)."></span></h3>
			<div class="accordion" id="akaCmdsMeaAcc"></div>
			<h3>Operations <span class="ui-icon ui-icon-help akaCmdGroupHelp helpToolTip" title="The order of operations commands is important.<br /><br /> If you use the same operations command twice, each command is performed based on its order of appearance. <br /><br />As IC reads query strings sequentially from left to right, the left-most command would be performed first. The second command would then be performed based on the output of the first command."></span></h3>
			<div class="accordion" id="akaCmdsOprAcc"></div>
			<h3>Output <span class="ui-icon ui-icon-help akaCmdGroupHelp helpToolTip" title="It is recommended that output commands follow operations
commands.<br /><br /> If you repeat an output command, only the right-most command is performed."></span></h3>
			<div class="accordion" id="akaCmdsOutAcc"></div>
		</div>
		<div id="cmdCont" class="fleft">
			<div id="cmdTestUrlCont" class="fleft">
				<div class="akaTitle" style="position: relative;"><label for="akaCmdUrl">Test URL</label> <span class="inBlock absRight"><button id="testCmds">Test</button></span></div>
...
-->
	<div class="akaTitle">Original Images (<span class="imgSizesList"></span> LtoR, hover to see details) <span class="inBlock absRight"><button id="origImagesToggle">show/hide</button></span></div>
	<div class="origImages checkboardBg sectionContainer">
	</div>
