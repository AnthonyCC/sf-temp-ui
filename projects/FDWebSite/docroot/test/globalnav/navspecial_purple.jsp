<!DOCTYPE html>
<%@ taglib uri="http://jawr.net/tags" prefix="jwr"%>
<html lang="en-US" xml:lang="en-US">
<head>
  <jwr:style src="/global.css"></jwr:style>
</head>
<body>
  <h1>Global checkout and gift card navigation</h1>

  <div class="globalNavigation">
      
    <p>GF Without links</p>
    
    <ul class="menu basic-skin purple-skin">
      <li class="menu-header item-skin-hovered">
        <span class="header-skin">gift cards</span>
      </li><!--
    --><li class="menu-item item-skin">      
        <span class="gf-item">gift & recipient options</span>
      </li><!--
      --><li class="menu-item item-skin">
        <span class="gf-item">payment info</span>
      </li><!--
      --><li class="menu-item item-skin">
        <span class="gf-item">confirmation</span>
      </li>
    </ul>
    
    <p>GF Second item</p>
    
    <ul class="menu basic-skin purple-skin">
      <li class="menu-header item-skin-hover">
        <a class="menu-header header-skin reset-link" href="/gift_cards">gift cards</a>
      </li><!--
    --><li class="menu-item item-skin item-skin-hovered gf-item">
          <span class="gf-item">gift & recipient options</span>
      </li><!--
      --><li class="menu-item item-skin">
          <span class="gf-item">payment info</span>
      </li><!--
      --><li class="menu-item item-skin">
          <span class="gf-item">confirmation</span>
      </li>
    </ul>
      
    <p>GF Third item</p>
    
    <ul class="menu basic-skin purple-skin">
      <li class="menu-header item-skin-hover">
        <a class="menu-header header-skin reset-link" href="/gift_cards">gift cards</a>
      </li><!--
    --><li class="menu-item item-skin item-skin-hover gf-item">
        <a class="gf-item reset-link" href="/options">gift & recipient options</a>
      </li><!--
      --><li class="menu-item item-skin item-skin-hovered">
          <span class="gf-item">payment info</span>
      </li><!--
      --><li class="menu-item item-skin">
          <span class="gf-item">confirmation</span>
      </li>
    </ul>
    
    <p>GF Fourth item</p>
    
    <ul class="menu basic-skin purple-skin">
      <li class="menu-header item-skin-hover">
        <a class="menu-header header-skin reset-link" href="/gift_cards">gift cards</a>
      </li><!--
    --><li class="menu-item item-skin item-skin-hover gf-item">
        <a class="gf-item reset-link" href="/options">gift & recipient options</a>
      </li><!--
      --><li class="menu-item item-skin item-skin-hover">
        <a class="gf-item reset-link" href="/payment">payment info</a>
      </li><!--
      --><li class="menu-item item-skin item-skin-hovered">
          <span class="gf-item">confirmation</span>
      </li>
    </ul>
  
  <h1>Checkout Main</h1>
  
  <ul class="menu basic-skin purple-skin">
    <li class="co-menu-header item-skin-hovered">
      <span class="header-skin">checkout</span>
    </li><!--
    --><li class="menu-item co-item-skin">
    
        <span class="co-item co-item1">view cart</span>
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item2">step 1:<br/> delivery address</span>
        
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item3">step 2:<br/> choose time</span>
        
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item4">step 3:<br/> payment info</span>
        
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item5">step 4:<br/> review & submit</span>
        
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item6">Standing Order</span>
        
    </li>
  </ul>  
  
  <p>Checkout 2</p>
  
  
  <ul class="menu basic-skin purple-skin">
    <li class="co-menu-header item-skin-hover">
        <a class="co-menu-header header-skin reset-link" href="./checkout">checkout</a>
    </li><!--
    --><li class="menu-item co-item-skin item-skin-hovered">
        <span class="co-item co-item1">view cart</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item2">step 1:<br/> delivery address</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item3">step 2:<br/> choose time</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item4">step 3:<br/> payment info</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item5">step 4:<br/> review & submit</span>
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item6">Standing Order</span>
        
    </li>
  </ul>   
  
    
  <p>Checkout 3</p>
  
  
  <ul class="menu basic-skin purple-skin">
    <li class="co-menu-header item-skin-hover">
          <a class="co-menu-header header-skin reset-link" href="./checkout">checkout</a>
    </li><!--
    --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item1 reset-link" href="./viewcart">view cart</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hovered">
          <span class="co-item co-item2">step 1:<br/> delivery address</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item3">step 2:<br/> choose time</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item4">step 3:<br/> payment info</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item5">step 4:<br/> review & submit</span>
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item6">Standing Order</span>
        
    </li>
  </ul>   
  
      
  <p>Checkout 4</p>
  
  <ul class="menu basic-skin purple-skin">
    <li class="co-menu-header item-skin-hover">
          <a class="co-menu-header header-skin reset-link" href="./checkout">checkout</a>
    </li><!--
    --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item1 reset-link" href="./viewcart">view cart</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item2 reset-link" href="./deliveryaddress">step 1:<br/> delivery address</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hovered">
          <span class="co-item co-item3">step 2:<br/> choose time</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item4">step 3:<br/> payment info</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item5">step 4:<br/> review & submit</span>
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item6">Standing Order</span>
        
    </li>
  </ul>   
  
        
  <p>Checkout 5</p>
  
  <ul class="menu basic-skin purple-skin">
    <li class="co-menu-header item-skin-hover">
          <a class="co-menu-header header-skin reset-link" href="./checkout">checkout</a>
    </li><!--
    --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item1 reset-link" href="./viewcart">view cart</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item2 reset-link" href="./deliveryaddress">step 1:<br/> delivery address</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hover">
    <a class="co-item co-item3 reset-link" href="./choosetime">step 2:<br/> choose time</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hovered">
          <span class="co-item co-item4">step 3:<br/> payment info</span>
    </li><!--
  --><li class="menu-item co-item-skin">
          <span class="co-item co-item5">step 4:<br/> review & submit</span>
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item6">Standing Order</span>
        
    </li>
  </ul>  
  
  <p>Checkout 6</p>
  
  <ul class="menu basic-skin purple-skin">
    <li class="co-menu-header item-skin-hover">
          <a class="co-menu-header header-skin reset-link" href="./checkout">checkout</a>
    </li><!--
    --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item1 reset-link" href="./viewcart">view cart</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hover">
          <a class="co-item co-item2 reset-link" href="./deliveryaddress">step 1:<br/> delivery address</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hover">
    <a class="co-item co-item3 reset-link" href="./choosetime">step 2:<br/> choose time</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hover">
    <a class="co-item co-item4 reset-link" href="./paymentinfo">step 3:<br/> payment info</a>
    </li><!--
  --><li class="menu-item co-item-skin item-skin-hovered">
          <span class="co-item co-item5">step 4:<br/> review & submit</span>
    </li><!--
  --><li class="menu-item co-item-skin">
        
          <span class="co-item co-item6">Standing Order</span>
        
    </li>
  </ul>
    
  </div>

</body>
</html>
