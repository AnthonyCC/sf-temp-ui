...
function JSONRpcClient_createMethod(methodName)
{
	...
	var responseHandlerName = "handleJSONResponse"; // Default Response Handler function name
	for(var i=0;i<arguments.length;i++) args.push(arguments[i]);
	if(typeof args[0] == "function") {
		callback = args.shift();
	}
	if(typeof args[0] == "object") {
		if (self.responseHandlerName) {
			responseHandlerName = self.responseHandlerName;					
		}
		if (typeof args[0][responseHandlerName] == "function") {
			callback = args.shift();
		}
	}
	...
	if(callback == null) {
		...
		if (typeof callback == "object") {
			req.responseHandlerName = responseHandlerName;
		}
...
function JSONRpcClient_async_handler()
{
	...
	try {
		...
		} else if (typeof res.cb == "object") {
			res.cb[res.responseHandlerName](res.result, res.ex, res.profile);
		}
	} catch(e) {
...
function JSONRpcClient_sendRequest(req)
{
	...
	var callURL = this.serverURL;
	if (this.setup_server_url && typeof(this.setup_server_url) == "function") {
		callURL = this.setup_server_url(req);
	}
    /* Send the request */
    if (typeof(this.user) == "undefined") {
	http.open("POST", callURL, (req.cb != null));
    } else {
	http.open("POST", callURL, (req.cb != null), this.user, this.pass);
    }
	...
	http.onreadystatechange = function() {
		...
		if (req.responseHandlerName) {
			res.responseHandlerName = req.responseHandlerName;
		}
