...
Modalbox.Methods = {
	...
	options: {
		...
		width: 50, // Default width in px
		height: 50, // Default height in px
	...
	setOptions: function(options) {
		...
		if (this.options.centered) this.options.transitions = false; // no transitions when centered
	},
	...
	_init: function(options) {
		...
		this.MBoverlay = new Element("div", {id: "MB_overlay", style: "opacity: "+this.options.overlayOpacity});
		...
		this.scrollObserver = this._scrollObserver.bindAsEventListener(this);
	...
	_scrollObserver: function() {
		...
		$('MB_window').setStyle({top:scrollOffsets[1] + 'px'});
		if (this.options.centered) this._setPosition();
	},
	...
	_hide: function(event) { // Internal hide method to use with overlay and close link
		...
		if (event.element().id == this.MBoverlay.id && !this.options.overlayClose) return false;
		this.hide();
	},
	...
	_insertContent: function(content, callback) {
		...
		// reset the position if centered
		if (this.options.centered) this._setPosition();
	},
	...
	_setPosition: function () {
		if (!this.options.centered)
			...
			var set_top = (this.options.top) ? this.options.top :
			Math.round(((window.innerHeight || document.documentElement.clientHeight ||
			document.body.clientHeight) - this.MBwindow.getHeight()) / 2 );
			var set_left = (this.options.left) ? this.options.left :
			Math.round(((window.innerWidth || document.documentElement.clientWidth ||
			document.body.clientWidth) - this.options.width) / 2 );
			Element.setStyle(this.MBwindow, {left: set_left + "px",	top: set_top +  "px"});
		}
 	},
