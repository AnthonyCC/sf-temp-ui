...
(function (fd) {
  ...
  var multiCustomizePopup = Object.create(POPUPWIDGET,{
    ...
    bodyTemplate: {
      value: expressco.multiCustomizePopup
    },
    scrollCheck: {
    ...
    emptyAtc: {
      value: function () {
        ...
        if(!items.length){
          ...
          fd.components.AddToCart.addToCart($els, {eventSource: 'FinalizingExternal', ignoreRedirect:true });
        }
    ...
    popupId: {
      value: 'xc-multi-customize-popup'
    },
    popupConfig: {
      value: {
        valign: 'bottom',
  ...
  if (FreshDirect.expressco.pendingCustomizations && Object.keys(FreshDirect.expressco.pendingCustomizations).length) {
    ...
    $(document).ready(function(){
      if (FreshDirect.expressco.pendingCustomizations && Object.keys(FreshDirect.expressco.pendingCustomizations).length) {
        multiCustomizePopup.open(FreshDirect.expressco.pendingCustomizations);
      }
    });
    ...
    $('#'+multiCustomizePopup.popupId).on('change', '[data-atc-required="true"]', function (e) {
        multiCustomizePopup.validate();
    });
  ...
  fd.modules.common.utils.register('expressco', 'multiCustomizePopup', multiCustomizePopup, fd);
}(FreshDirect));
