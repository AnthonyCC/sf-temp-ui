...
(function(fd) {
  ...
  /**
   * check if passed in elemenet is fully inside the viewport
   * returns true/false
   */
  utils.isInViewPort = function(elem) {
    if (!elem || elem.getBoundingClientRect === undefined) { return false; }
  ...
  /* get mouse position offset (relative to page) from mouse event */
  utils.getCursorOffset = function(event){
  ...
  /* get document scroll offset */
  utils.getDocumentScrollOffset = function() {
    return { 
  ...
  /* measure an element, return it's offset sizing (height and width only)
   *  if no element is passed in:
   *    auto-measures letters for the widest in the current font and returns that as measurement
   *    the return object will also include the letter found as 'value'
   */
  utils.getElemSizing = function(elem) {
    if (!elem) { return utils.getWidestLetter() }
    ...
    offset = tempElem.getBoundingClientRect();
    
    tempElem = void 0; //cleanup
  ...
  /* loops over upper and lower case A-Z to find the widest rendered letter */
  utils.getWidestLetter = function() {
    var capsIndex = 65, smallIndex = 97;
    ...
    for (var i = capsIndex; i < capsIndex + 26; i++) {
      tempDiv.innerText = String.fromCharCode(i);
      var computedWidth = window.getComputedStyle(tempDiv, null).getPropertyValue("width");
      if(highestWidth < parseFloat(computedWidth)) {
        highestWidth = parseFloat(computedWidth);

        returnObj.value = String.fromCharCode(i);
        returnObj.height = parseFloat(window.getComputedStyle(tempDiv, null).getPropertyValue("height"));
        returnObj.width = highestWidth;
      }
    }
