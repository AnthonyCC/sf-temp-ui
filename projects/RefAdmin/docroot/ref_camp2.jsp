...
function checkError()
{
  ...
  if(document.rp.errMsg1.value!="" && document.rp.errMsg1.value=="false")
  {
     ...
     document.location.href = 'ref_camp1.jsp?SORT_COLUMN_NME='+sortColumnName+'&START_INDEX='+startIndex;
  }     
}
...
function clearAll(txt)
{
    ...
    if(txt!="save")
    {
       document.rp.refCampName.value="";   
    }   
    document.rp.refCampDesc.value="";
    document.forms[0].referralObjective.selectedIndex = 0;
}
...
<font color="green">Referral Campaign</font>
...
<div class="app">
<%
   String selectRefCamp=(String)request.getParameter("selectRefCamp");
   if(selectRefCamp==null) selectRefCamp="";
   
%>

<form name="rp" method="POST"  action="ref_camp2.jsp">
<input type="hidden" name="pageName" value="ref_camp2"/>
<input type="hidden" name="actionName" value=""/>
<input type="hidden" name="selectRefCamp" value="<%=selectRefCamp%>"/>
...
<%
  ...
  <ref:referralCampaign id="refCamp" pageName="ref_camp2">          

<input type="hidden" name="refCampId" value="<%=refCamp.getPK()==null?"":refCamp.getPK().getId()%>"/>
...
<tr>
 <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td> 
 <td colspan="2" align="center"> <b><U> REFERRAL CAMPAIGN <UL><b></td> 
 <td></td> 
 <td></td> 
 <td></td> 
</tr>
...
<%
    ...
    //System.out.println("result"+result+"refObj"+refObj);
%>
...
<tr>
 <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td> 
 <td colspan="2" align="center"> <font color="red">*</font> <b> CAMPAIGN NAME : <b></td> 
 <td> 
 <%
if("save".equalsIgnoreCase(actionType))
   {
%>
<input type="text" name="refCampName" value="<%=refCamp.getName()%>" readOnly="true" />
<%
}else{
%>
<input type="text" name="refCampName" value="<%=refCamp.getName()%>" maxlength="55"/>
<%
}
%>
...
<tr>
 <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td> 
 <td colspan="2" align="center"><font color="red">*</font> <b> CAMPAIGN DESC: <b></td> 
 <td> 
 <textarea name="refCampDesc" rows="3" cols="40" style="width: 220px"  onKeyDown="textCounter(this.form.refCampDesc,255);" onKeyUp="textCounter(this.form.refCampDesc,255);"><%=refCamp.getDescription()%></textarea>
...
<tr>
 <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td> 
 <td colspan="2" align="center"><font color="red">*</font> <b> OBJECTIVE :</b></td> 
 <td> <select name="referralObjective">
       <option value=""></option>
      <%
             Set collection=FDReferralProgramManager.getAllReferralObjectives();
             Iterator iterator=collection.iterator();
             while(iterator.hasNext())
             {
             ReferralObjective objective=(ReferralObjective)iterator.next();
      %>
      <option value="<%=objective.getPK().getId()%>" <%= objective.getName().equalsIgnoreCase(refCamp.getObjective().getName())?"selected":"" %> ><%=objective.getName()%></option>
      <%
             }
       %>                         
       </select>
       <ref:ErrorHandler result='<%= result %>' name='referralObjective' id='errorMsg'><span class="error_detail"><%=errorMsg%></span></ref:ErrorHandler>
 </td> 
 <td></td> 
 <td></td> 
</tr>
...
<td align="left"><INPUT type="button" value=" back " onClick="location.href='ref_camp1.jsp?SORT_COLUMN_NME=<%=sortByColumnName%>&START_INDEX=<%=startIndex%>'"/></td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
