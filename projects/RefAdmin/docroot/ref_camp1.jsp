...
<div class="bodycol">
<div class="app">
<form method="post" name="rp" action="ref_camp1.jsp">

<ref:referralProgramSummary id="refCampSum" pageName="ref_camp1">                 
   <%
...
<input type="hidden" name="pageName" value="ref_camp1"/>
...
<table width="80%" cellpadding="8" cellspacing="0" border="0">
	<tr bgcolor="lightgrey">
        ...
        <td colspan="2">
        <%  if(!"CAMPAIGN_NAME".equalsIgnoreCase(sortByColumnName)){        %>
        <b> <a href="javascript:getSortedReferralProgram('CAMPAIGN_NAME');"> Campaign Name </a><b>
        <% }else{  %>
        <b>  Campaign Name <b>
        <% } %>                        
        </td>                    
        ...
        <td >
        <%  if(!"REF_OBJ_ID".equalsIgnoreCase(sortByColumnName)){        %>
        <b> <a href="javascript:getSortedReferralProgram('REF_OBJ_ID');"> Referral Objective </a><b>
        <% }else{  %>
        <b>  Referral Objective <b>
        <% } %>                                
        </td>    
        <td > </td>    
        <td >  </td>    
...
<logic:iterate id="rp" collection="<%=refCampSum%>" type="com.freshdirect.fdstore.referral.ReferralCampaign" indexId="counter" >    
     <tr> 
        <td >   </td>    
		<td> <%=rp.getPK().getId() %></td>
        <td colspan="2"><b3>  <a href="ref_camp2.jsp?SORT_COLUMN_NME=<%=sortByColumnName%>&START_INDEX=<%=startIndex%>&actionName=update&selectRefCamp=<%=rp.getPK().getId() %>"> <%=rp.getName()%> </a><b3></td>    
        <td > <%=rp.getDescription()%> </td>            
        <td > <%=rp.getObjective()!=null?rp.getObjective().getName():""%> </td>            
        <td ></td>            
        <td > </td>            
    </tr>
    </logic:iterate>
...
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="ref_camp2.jsp?SORT_COLUMN_NME=<%=sortByColumnName%>&START_INDEX=<%=startIndex%>">Click here to create new Referral Campaign</a> 
