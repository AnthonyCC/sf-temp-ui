...
<div class="bodycol">
<div class="app">
<form method="post" name="rp" action="ref_obj1.jsp">

<ref:referralProgramSummary id="refObjSum" pageName="ref_obj1"> 
   <%
...
<input type="hidden" name="pageName" value="ref_obj1"/>
...
<table width="80%" cellpadding="8" cellspacing="0" border="0">
	<tr bgcolor="lightgrey">
        ...
        <td colspan="2">
        <%  if(!"OBJECTIVE_NAME".equalsIgnoreCase(sortByColumnName)){        %>
        <b> <a href="javascript:getSortedReferralProgram('OBJECTIVE_NAME');"> Objective Name </a><b>
        <% }else{  %>
        <b>  Objective Name <b>
        <% } %>                
        </td>                    
...
%>
   ...
   <logic:iterate id="rp" collection="<%=refObjSum%>" type="com.freshdirect.fdstore.referral.ReferralObjective" indexId="counter" >
  
   
    <tr> 
        <td >   </td>    
		<td> <%=rp.getPK().getId() %></td>
        <td colspan="2"><b3> <a href="ref_obj2.jsp?SORT_COLUMN_NME=<%=sortByColumnName%>&START_INDEX=<%=startIndex%>&actionName=update&selectRefObj=<%=rp.getPK().getId() %>"> <%=rp.getName()%> </a><b3></td>    
...
<br>
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="ref_obj2.jsp?SORT_COLUMN_NME=<%=sortByColumnName%>&START_INDEX=<%=startIndex%>">Click here to create new Referral Objective</a> 
