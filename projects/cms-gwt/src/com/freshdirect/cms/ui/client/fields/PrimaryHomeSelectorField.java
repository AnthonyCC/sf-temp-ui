...
public class PrimaryHomeSelectorField extends OneToManyRelationField {
	...
	public PrimaryHomeSelectorField(GwtNodeContext contexts, Map<String,Set<String>> mapping, String storeKey) {
		super("PRIMARY_HOME", allowedTypes, false, false, "Product");
	...
	private ToolButton addPrimaryHomeButton;
	
	@Override
	protected void setupTopToolbar(ToolBar aToolBar) {
		addPrimaryHomeButton = new ToolButton( "add-relation" );
		addPrimaryHomeButton.setToolTip( new ToolTipConfig( "ADD", "Add a relation..." ) );
	}
	...
	protected List<ColumnConfig> setupExtraColumns() {
		...
		// [change] button
		{
			...
			cfg.setRenderer(new GridCellRenderer<OneToManyModel>() {
				@Override
				public Object render(final OneToManyModel model, String property,
						ColumnData config, int rowIndex, int colIndex,
						final ListStore<OneToManyModel> store, Grid<OneToManyModel> grid) {
		...
		for (OneToManyModel m : parentz) {
			...
			for (String sKey : mapping.keySet()) {
				if (mapping.get(sKey).contains(_pKey)) {
					final boolean flag = sKey.equals(storeKey);
