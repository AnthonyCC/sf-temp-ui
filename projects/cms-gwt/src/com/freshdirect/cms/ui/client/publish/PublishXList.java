...
public class PublishXList extends LayoutContainer {
    ...
    protected class NewPublishWidget extends Html {
        ...
        public NewPublishWidget() {
            super("<div><div class=\"publish-history-show-data-true\">" + "<div class=\"publish-history-id\">Current publish</div>" + "</div></div>");
    ...
    protected class ProgressPublishWidget extends Html {
        ...
        public void setPublish(GwtPublishData data) {
            ...
            createProgressTimer();
            if (data != null) {
                setHtml("<div class=\"" + data.getStatus() + "\">" + "<div class=\"publish-history-id\">#" + data.getId() + "</div>" + "<div class=\"publish-history-comment\">"
                        + data.getComment() + "</div>" + "<div class=\"publish-history-info\"><b>" + data.getPublisher() + "</b><br>" + data.getCreated() + "</div>" + "</div>");
                resumeTimer();
            } else {
                setHtml("<div></div>");
                stopTimer();
                            ...
                            CmsGwt.getContentService().getPublishDataX(query, new BaseCallback<GwtPublishData>() {
                                ...
                                public void onSuccess(GwtPublishData result) {
                                    if (!result.isInProgress()) {
                                        ...
                                        if (isSelected()) {
                                            FeedPublishView.getInstance().setOverview(result);
                                        }
                                    }
                                }
                            });
        ...
        publishHistory.addListener(Events.OnClick, new Listener<BaseEvent>() {
            ...
            public void handleEvent(BaseEvent be) {
                ...
                if (selectedItem.isShowNext()) {
                    CmsGwt.getContentService().getPublishHistoryByType(new BasePagingLoadConfig(loadedItemsCount, GwtPublishData.FETCH_SIZE), "FEED",
