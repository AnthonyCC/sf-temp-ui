...
We are very sorry to hear about your damaged items. We will address this issue further to ensure that the products are assembled more efficiently to prevent such occurrences in the future. A credit has been issued for the product and we apologize for any inconvenience. 
...
$<xsl:value-of select='format-number(amount, "###,##0.00", "USD")'/> from the <xsl:value-of select="$deptName"/> Department in store credit
</xsl:if></xsl:if>
</xsl:for-each>
...
$<xsl:value-of select='format-number(amount, "###,##0.00", "USD")'/><xsl:text> </xsl:text>from the <xsl:value-of select="$deptName"/> Department charged back to your credit card
