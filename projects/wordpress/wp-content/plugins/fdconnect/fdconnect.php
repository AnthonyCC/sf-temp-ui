<?php
/*
 Plugin Name: FD Connect
 Author: Tamas Gelesz
 Author URI:
 Plugin URI:
 Description: Integrate FD and Wordpress.  Provides single-signon using oAuth. This is a customization of the Twit Connect Plugin 2.57 by Shannon Whitley
 Acknowledgments:
...
//FD Connect filters
//add_action('login_form_autologout', 'fdc_autologout');
...
function twc_init()
{
	...
	else
	{
		if ( isset($_GET['page']) ) {
			...
			if($plugin_page == 'fdconnect/fdconnect.php')
			{
...
function twc_login_form()
{
	...
	{
		echo '<link rel="stylesheet" href="'.WP_PLUGIN_URL . '/fdconnect/style.css" type="text/css" media="all" />';
...
function twc_stylesheet_add() {
	$src = WP_PLUGIN_URL . '/fdconnect/style.css';
...
*/
		$avatar ="<img src='".get_userdata($id_or_email)->description."' class='avatar avatar-40' width='40'/>";				
		return $avatar;
	} else {
		return $avatar;
	}
...
//* twc_oAuth_Start
//************************************************************************************
function twc_oAuth_Start($autoAuth = FALSE)
{
	...
	if (strlen($token) < 1){
		echo '<script type="text/javascript">location.href = "404.php?oautherror";</script>';
	}
	...
	$authUrl=$to->getAuthorizeURL($token, $url);
	
	if ($autoAuth){
		$authUrl = $authUrl."&autoauth=true";
	}
	
	echo '<script type="text/javascript">location.href = "'.$authUrl.'";</script>';
}
...
function twc_oAuth_Confirm()
{
	...
	try {
		$xml = $to->get(get_option('fdc_fd_storefront_base').'/oauth/user_data');
		$twitterInfo = new SimpleXMLElement($xml);

		$id = $twitterInfo->id;
		$screen_name = $twitterInfo->screen_name;
		$name = $twitterInfo->name;
		$avatar_url = $twitterInfo->avatar_url;
		twc_Login($id.'|'.$screen_name.'|'.$name.'|'.$avatar_url);
	} catch (Exception $e) {
		error_log($e);
	}
}
...
function twc_Login($pdvUserinfo) {
	...
	//Use the url from the FD profile.
	$user_avatar_url = $userinfo[3];
...
*/
	...
	//user avatar url is stored in description field
	$userdata = array(
    'user_pass' => wp_generate_password(),
    'user_login' => $user_login_n_suffix,
    'display_name' => $userinfo[2],
    'description' => $user_avatar_url,
    'user_email' => $userinfo[0]//$twc_email_default
	);
	...
	if(!$wpuid)
	{
		...
		//for existing admins, who registered in advance
		$wpuid = email_exists($userinfo[0]);
		if (!$wpuid)		
		{
...
//		}
	...
	else
	{
		...
		if($user_obj->display_name != $userinfo[2] || $user_obj->description != $user_avatar_url)
		{
			$userdata = array(
        'ID' => $wpuid,
        'display_name' => $userinfo[2],
...
{
	add_submenu_page("options-general.php", "FD Connect",
		"FD Connect", 10, __FILE__, "twitconnect_configuration");
}
...
//*****************************************************************************
//* fdc_comment_form_defaults
//*****************************************************************************
function fdc_comment_form_defaults($defaults)
{
...
//* fdc_comment_form_defaults
//*****************************************************************************
function fdc_login_url($login_url, $redirect)
{
	global $twc_url, $twc_page;

	$login_url = $twc_url.'/'.$twc_page .'&loc='. urlencode($redirect);
	return $login_url;
}
...
KEEPME4;
	...
	if (isset($_POST["twc_save"])) {
		...
		update_option('fdc_fd_storefront_base', stripslashes($_POST["fdc_fd_storefront_base"]) );
		update_option('fdc_oasconfig_serverdomain', stripslashes($_POST["fdc_oasconfig_serverdomain"]) );
		update_option('fdc_oasconfig_sitepage', stripslashes($_POST["fdc_oasconfig_sitepage"]) );
	}
...
<h3>FD Connect Configuration</h3>
<form action='' method='post' id='twc_conf'>
	<div id="twc_tabs">
		<ul>
			<li><a href="#twc_tab1">Settings</a></li>
			<?php /*
		...
		<div id="twc_tab1">
			...
			<td><?php if(!version_compare(PHP_VERSION, '5.0.0', '<')) : ?>
			<table>
			<tr>
			<td valign="top" width="5%"><input type='checkbox' name='twc_local' value='Y' <?php echo $twc_local ?> /></td>
			<td valign="top"><small>Check this box to use your own Consumer Key and Consumer Secret.</small> <br /> <small>For this option, you must register a new application at <a
			href="http://dev.twitter.com/apps/new">Twitter.com</a>. Enter the url for your blog in the Application Website and Callback URL.</small> <br /> <small>Help in filling out the registration
			can be found on the <a href="http://www.voiceoftech.com/swhitley/?page_id=706">Twit Connect</a> page.</small>
			</td>
			</tr>
			</table>
			</td>
			...
			*/?>
				<tr>
					...
					<td><input type='text' name='twc_consumer_key' value='<?php echo $twc_consumer_key ?>' size="50" /> <?php /* <br /> <small> (Optional) Your application consumer key from Twitter.com. </small>*/?>
					</td>
				</tr>
				<tr>
					<td width="20%" valign="top">Consumer Secret</td>
					<td><input type='text' name='twc_consumer_secret' value='<?php echo $twc_consumer_secret ?>' size="50" /> <?php /* <br /> <small> (Optional) Your application consumer secret from Twitter.com. </small>*/?>
					</td>
				</tr>
				<tr>
					<td width="20%" valign="top">Storefront Base URL</td>
					<td><input type='text' name='fdc_fd_storefront_base' value='<?php echo get_option('fdc_fd_storefront_base') ?>' size="50" /> <?php /* <br /> <small> (Optional) Your application consumer secret from Twitter.com. </small>*/?>
					</td>
				</tr>
				<tr>
					<td width="20%" valign="top">OAS Server Domain</td>
					<td><input type='text' name='fdc_oasconfig_serverdomain' value='<?php echo get_option('fdc_oasconfig_serverdomain') ?>' size="50" /> <?php /* <br /> <small> (Optional) Your application consumer secret from Twitter.com. </small>*/?>
					</td>
				</tr>
				<tr>
					<td width="20%" valign="top">OAS Site Page</td>
					<td><input type='text' name='fdc_oasconfig_sitepage' value='<?php echo get_option('fdc_oasconfig_sitepage') ?>' size="50" /> <?php /* <br /> <small> (Optional) Your application consumer secret from Twitter.com. </small>*/?>
					</td>
				</tr>
			</table>
			<?php /*
