...
$.widget(widgetName, {
	...
	_initEvents: function() { //initialize all event listeners
		...
		this.input.bind("keydown keypress keyup", function(event) {
			...
			case keyCodes.DOWN:
				//  change: Modified so Down Arrow selects next item; Alt + Down Arrow opens dropdown
				if (event.altKey) {
					self.showList();
				}
				else {
			...
			default:
				// change: Modified to support showAndFilterOnKeyUp option
				if (self.options.showAndFilterOnKeyUp) {
					self.showList();
		...
		this.input.bind("click", function(e) {
			...
			if (!self.listVisible()) { 
				...
				if (self.internalFocus && self.options.showListOnFocus) {
					self.showList();
				}
	...
	realFocusEvent: function() {
		...
		// change: Removed to support showListOnFocus option
		//this.showList();
		//  change: End Removed
		
		//  change: Added to support showListOnFocus option
		if (self.options != null && self.options.showListOnFocus) {
			this.showList();
		}
		//  change: End Added
	},
	...
	//  change: Modified to add parameter to not select all text
	inputFocus: function(noSelect) {
...
$.extend($.ui.ufd, {
	...
	defaults: { // 1.7 default options location, see below
        ...
        //  change: Added showListOnFocus and showAndFilterOnKeyUp options
        // showListOnFocus changes are from http://code.google.com/p/ufd/issues/detail?id=41
        showListOnFocus: false, // display list on focus (tab or click)
        showAndFilterOnKeyUp: true, // display and filter list as user types
        //  change: End Added
		...
		uiCss: {
			skin: "uiCss", 
			input: "ui-widget-content idlefield",
			inputDisabled: "disabled",
			...
			listWrapper: "list-wrapper ui-widget ui-widget-content",
			listWrapperUp: "list-wrapper-up",			
