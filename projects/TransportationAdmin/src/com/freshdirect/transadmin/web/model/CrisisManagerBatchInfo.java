...
public class CrisisManagerBatchInfo implements java.io.Serializable {
	...
	public CrisisManagerBatchInfo(ICrisisManagerBatch batch) {
		...
		if(this.batch.getOrder() != null) {
			for(ICrisisManagerBatchOrder _orderInfo : this.batch.getOrder()) {
	...
	public String getDeliveryDate() {
		...
		try {			
			strBuf.append("<b>Selected Date: </b>"+ TransStringUtil.getDate(batch.getDeliveryDate())).append("<br/>");
	...
	public String getCutOffInfo() {
		...
		try {			
			...
			if(batch.getDeliveryType() != null && batch.getDeliveryType().length > 0){
				...
				strBuf.append("Delivery Type: <b>ALL</b><br/>");
			}
		} catch (ParseException e) {
			return null;
		}
