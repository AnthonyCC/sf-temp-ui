...
/**
 * This class will be used for populating the country list by warehouse for each product via Jco-server registered to
 * ERP system
 *
...
public class FDCOOLJcoServer extends FdSapServer
{
		...
		public void handleRequest(final JCoServerContext serverCtx, final JCoFunction function)
		{
			...
			try
			{
				...
				for (int i = 0; i < coolTable.getNumRows(); i++)
				{
					...
					final String plant = SapProperties.getPlant(); // FDSapHelperUtils.getString(coolTable.getString("WERKS"));
					...
					{
						LOG.debug("Got Cool Record Plant: " + plant + "\t" + "MatNo: " + matNo + "\t" + "country1: " + country1 + "\t"
								+ "country2: " + country2 + "\t" + "country3: " + country3 + "\t" + "country4: " + country4 + "\t"
								+ "country5:" + country5);
					}
				...
				exportParamList.setValue("MESSAGE", String.format("Product Cool info updated successfully! [ %s ]", new Date()));
			...
			catch (final Exception e)
			{
				...
				exportParamList.setValue("MESSAGE",
						"Error importing  country of origin: " + e.toString().substring(0, Math.min(225, e.toString().length())));
			}
		}
	}
			...
			final FDJcoServerResult result) throws LoaderException
	{
		...
		try
		{
			for (final Map.Entry<String, Map<String, List<String>>> materialEntry : countryOfOriginMap.entrySet())
			{
				...
				ErpCOOLInfo coolInfoModel = new ErpCOOLInfo(matNo, null, null);
				erpCOOLInfoList.add(coolInfoModel);

				for (final Map.Entry<String, List<String>> materialWarehouseEntry : materialEntry.getValue().entrySet())
				{
					//final String warehouseCode = materialWarehouseEntry.getKey();					
					coolInfoModel.setCountryInfo(materialWarehouseEntry.getValue());
				}
			}	
	...
	private List<String> getCountryInfo(String country1, String country2, String country3, String country4, String country5) {
		...
		if (!StringUtil.isEmpty(country3) && !StringUtil.isEmpty(country2)
				&& !StringUtil.isEmpty(country1))
		{
			countryInfo.add(country3);
		}
		if (!StringUtil.isEmpty(country4) && !StringUtil.isEmpty(country3)
				&& !StringUtil.isEmpty(country2)
				&& !StringUtil.isEmpty(country1))
		{
			countryInfo.add(country4);
		}
		if (!StringUtil.isEmpty(country5) && !StringUtil.isEmpty(country4)
				&& !StringUtil.isEmpty(country3)
				&& !StringUtil.isEmpty(country2)
				&& !StringUtil.isEmpty(country1))
		{
