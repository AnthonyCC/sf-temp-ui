...
/**
 * This class represents the processing of group scale details sent by SAP to Store front
 * 
 * @author kkanuganti
 *
 */
public class FDGroupScalePriceJcoServer extends FdSapServer
{
	...
	protected JCoRepository createRepository()
	{
		...
		tableMetaDataList.add(new TableMetaData("ZGROUP_SHORT_DESC", JCoMetaData.TYPE_CHAR, 30, 0, "Short Description"));
		...
		tableMetaDataList.add(new TableMetaData("ZSGRP_SUOM", JCoMetaData.TYPE_CHAR, 3, 0, "Group Scale Price Sales UOM"));
		tableMetaDataList.add(new TableMetaData("ZSGRP_EXP_IND", JCoMetaData.TYPE_CHAR, 1, 0, "Expiry Indicator"));
		tableMetaDataList.add(new TableMetaData("ZSGRP_QTY_FLAG", JCoMetaData.TYPE_CHAR, 1, "Qty 1 allowed flag"));
	...
	protected class FDConnectionHandler extends FDSapFunctionHandler implements JCoServerFunctionHandler
	{
		...
		public void handleRequest(final JCoServerContext serverCtx, final JCoFunction function)
		{
			...
			try
			{
				...
				final Map<String, List<GroupScalePriceParameter>> groupScaleRecordMap = new HashMap<String, List<GroupScalePriceParameter>>();
				...
				if (FDJcoServerResult.OK_STATUS.equals(result.getStatus()))
				{
					...
					exportParamList.setValue("MESSAGE", String.format("Group scale details imported successfully [ %s ]", new Date()));
				}
			}
			catch (final Exception e)
			{
				...
				exportParamList.setValue("MESSAGE",
						"Error importing group scale " + e.toString().substring(0, Math.min(230, e.toString().length())));
			}
				...
				final FDJcoServerResult result) throws LoaderException
		{
			...
			for (final Map.Entry<String, List<GroupScalePriceParameter>> groupScaleRecordEntry : groupScaleRecordMap.entrySet())
			{
				...
				for (final GroupScalePriceParameter scaleGroupRecord : groupScaleRecordEntry.getValue())
				{
					final Boolean isScaleGroupExpired = toBooleanObject(
							StringUtils.lowerCase(groupScaleRecordEntry.getValue().get(0).getGrpExpiryIndicator()), "x", null, null);
					...
					if (Boolean.FALSE.equals(isScaleGroupExpired))
					{
						...
						{
							//[ERROR:Same material in more than one scale group in the export]
							result.addError(scaleGroupRecord.getMaterialID(), "Appeared in more than one active group in the batch");
						}
						else
						{
					...
					String existingScaleGroupId = checkIfMaterialAlreadyExistsInActiveGroup(scaleGroupId, scaleGroupRecord.getMaterialID());
					if (existingScaleGroupId != null && existingScaleGroupId.length() > 0)
					{
						...
						activeScaleGroupToMaterialMapping.get(existingScaleGroupId).add(scaleGroupRecord.getMaterialID());
					}	
				}
			}
			...
			if (activeScaleGroupToMaterialMapping.size() > 0)
			{
						...
						.entrySet())
				{
					if (activeScaleGroupToMaterialMapping.containsKey(groupScaleRecordEntry.getKey()))
					{
						...
						//remove the group from active group map if active group is expired or if this group no more contains the material being checked against. 
						...
						else
						{
							for (final GroupScalePriceParameter scaleGroupRecord : groupScaleRecordEntry.getValue())
							{
								if (activeScaleGroupToMaterialMapping.get(groupScaleRecordEntry.getKey()) != null 
										&& !activeScaleGroupToMaterialMapping.get(groupScaleRecordEntry.getKey()).contains(
		 ...
		 * @throws Exception
		 */
		private void populateGroupScale(final Map<String, List<GroupScalePriceParameter>> groupScaleRecordMap) throws Exception
		{
			...
			try
			{
						...
						.entrySet())
				{
					...
					for (final GroupScalePriceParameter scaleGroupRecord : scaleGroupRecordEntry.getValue())
					{
						...
						ErpGrpPriceZoneModel scaleGroupPriceRow = new ErpGrpPriceZoneModel(
								scaleGroupRecord.getPricingZoneId(), 
		...
		private GroupScalePriceParameter populateGroupScaleRecord(final JCoTable groupTable)
		{
			...
			param.setGrpShortDesc(FDSapHelperUtils.getString(groupTable.getString("ZGROUP_SHORT_DESC")));
			...
			{
				LOG.debug("Got scale group record for Material No:" + param.getMaterialID() + "\t GroupId:" + param.getGroupId()
						+ "\t SalesOrg:" + param.getSalesOrganizationId() + "\t DistChannel:" + param.getDistributionChannelId()
						+ "\t ZoneId:" + param.getPricingZoneId() + "\t UnitCode:" + param.getPricingUnitCode() + "\t ScaleUnitCode:"
	...
	private String checkIfMaterialAlreadyExistsInActiveGroup(String grpId, String matId) {
		...
		{
			throw new EJBException("Failed to validate if Material Already Exists In Active Group: " + matId + "Exception Msg: " + ex.toString());
		} 
		finally {
			if (ctx != null) {
				try {
