...
/**
 * This class represents the processing of inventory / stock details sent by SAP to Store front
 * 
 * @author kkanuganti
 *
 */
public class FDInventoryJcoServer extends FdSapServer
{
	...
	protected JCoRepository createRepository()
	{
		...
		tableMetaDataList.add(new TableMetaData("WERKS", JCoMetaData.TYPE_CHAR, 4, 0, "Plant ID"));//4,18
		...
		tableMetaDataList.add(new TableMetaData("BMENG", JCoMetaData.TYPE_BCD, 7, 3, "Confirmed Qty")); // 13.3 BCD, length 7
	...
	protected class FDConnectionHandler extends FDSapFunctionHandler implements JCoServerFunctionHandler
	{
		...
		public void handleRequest(final JCoServerContext serverCtx, final JCoFunction function)
		{
			...
			try
			{
				...
				final Map<String, Map<String, List<ErpInventoryEntryModel>>> stockMap = new HashMap<String, Map<String, List<ErpInventoryEntryModel>>>();
				for (int i = 0; i < materialTable.getNumRows(); i++)
				{
					...
					final double commitedQty = materialTable.getDouble("BMENG");
					final String salesUnit = FDSapHelperUtils.getString(materialTable.getString("MEINS"));

					if (LOG.isDebugEnabled())
					{
						LOG.debug("Got Inventory Record For Plant:" + plant + "\t Material No:" + matNo + "\t Start Date:" + startDate
								+ "\t Committed Qty:" + commitedQty + "\t Sales Unit:" + salesUnit);
					}
					
					// do not process batch if any material fails as SAP resends all inventory data for the batch
					if(SapProperties.getPlant() != null && SapProperties.getPlant().equalsIgnoreCase(plant))
					{
						if (!stockMap.containsKey(matNo))
						{
							stockMap.put(matNo, new HashMap<String, List<ErpInventoryEntryModel>>());
						}
						if (!stockMap.get(matNo).containsKey(plant))
						{
							stockMap.get(matNo).put(plant, new ArrayList<ErpInventoryEntryModel>());
						}

						stockMap.get(matNo).get(plant).add(new ErpInventoryEntryModel(startDate, commitedQty));
					}
					else
					{
						LOG.debug("Unknown Plant" + "\t" +   plant + "\t" +  matNo + "\t" + startDate + "\t" + commitedQty + "\t" + salesUnit);
					}
				...
				if(stockMap.size() > 0)
				{
					...
					exportParamList.setValue("MESSAGE", String.format("Product inventory updated successfully! [ %s ]", new Date()));
				} 
				else 
				{
					exportParamList.setValue("RETURN", "W");
					exportParamList.setValue("MESSAGE", String.format("No Product inventory to process. Please check the export data [ %s ]", new Date()));
				}
			}
	 ...
	 */
	private void processStocklevels(final Map<String, Map<String, List<ErpInventoryEntryModel>>> stockMap) throws LoaderException
	{
		...
		try
		{
			...
			for (final Map.Entry<String, Map<String, List<ErpInventoryEntryModel>>> materialEntry : stockMap.entrySet())
			{
				final String matNo = materialEntry.getKey();
				for (final Map.Entry<String, List<ErpInventoryEntryModel>> materialWarehouseEntry : materialEntry.getValue().entrySet())
				{
					//final String warehouseCode = materialWarehouseEntry.getKey();
					stockEntries.add(new ErpInventoryModel(matNo, currentDate, materialWarehouseEntry.getValue()));
				}
			}
	 ...
	 * @throws EJBException
	 */
	private void updateInventories(List<ErpInventoryModel> stockEntries)
			throws EJBException
	{
		Context ctx = null;
		try
