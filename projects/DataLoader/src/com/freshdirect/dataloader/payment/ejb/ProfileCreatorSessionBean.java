...
public class ProfileCreatorSessionBean extends SessionBeanSupport {
	...
	public void createProfiles(String batchId) throws SQLException, FDResourceException   {
		...
		try {
			...
			LOGGER.info("processing profile creation for batch id: " + batchId + " Total no of customers: "
					+ customers.size());
			...
			for (String customer : customers) {
				...
				if (conn.isClosed())
					conn = getConnection();
				boolean hasProfile = ProfileCreatorDAO.getCustomerProfile(conn, identity.getFDCustomerPK(),
						"siteFeature.Paymentech");
				...
				if (!hasProfile) {
					...
					try {
						...
						e.printStackTrace();
						output = new ProfileCreatorOutput(customer, insertTimestamp, false,
								"FAILED to load payment methods for customerID : " + customer + " with exception :"
										+ e.toString());
						paymentMethods = Collections.emptyList();
					}
					LOGGER.info("CustomerID : " + customer + " has " + paymentMethods != null ? paymentMethods.size()
							: 0 + " payment methods in their account.");
					...
					for (ErpPaymentMethodI paymentMethod : paymentMethods) {
						...
						try {
							if ((EnumPaymentMethodType.CREDITCARD.equals(paymentMethod.getPaymentMethodType())
								...
								LOGGER.info("Attempting to add profile for paymentMethodID :" + paymentMethodId
										+ " of Type :" + paymentMethod.getPaymentMethodType());
								...
								if (response != null && response.isRequestProcessed()) {
									if (response.getBillingInfo() != null
											&& response.getBillingInfo().getPaymentMethod() != null) {
										profileId = response.getBillingInfo().getPaymentMethod().getBillingProfileID();
										outputDtlList.add(new ProfileCreatorOutputDetail(customer, insertTimestamp,
												paymentMethodId, profileId, "SUCCESS", null));
										LOGGER.info("Profile addition SUCCESS for paymentMethodID :" + paymentMethodId
												+ " with ProfileID :" + profileId);
									} else {
										outputDtlList.add(new ProfileCreatorOutputDetail(customer, insertTimestamp,
												paymentMethodId, null, "FAILURE",
												response.getResponseCode() + "-" + response.getStatusMessage()));
									...
									outputDtlList.add(new ProfileCreatorOutputDetail(customer, insertTimestamp,
											paymentMethodId, null, "FAILURE",
											"response is null or request not processed by gateway"));
									error = true;
								}
							} else {
								outputDtlList.add(new ProfileCreatorOutputDetail(customer, insertTimestamp,
										paymentMethodId, null, "SKIPPED", null));
							}
						} catch (Exception e) {
							e.printStackTrace();
