...
import com.freshdirect.dataloader.payment.reconciliation.paypal.SectionFooterDataRecord;

public class SectionFooterParser extends PayPalSettlementParser implements FooterDataParserHelper {
	
	private final EnumPayPalRecordType type;
	
	private static final String SECTION_RECORD_COUNT = "SECTION_RECORD_COUNT";
	
	public SectionFooterParser(EnumPayPalRecordType type){
		super();
		this.type = type;
		...
		fields.add(new Field(SECTION_RECORD_COUNT, 26, true));
	}
	
	@Override
	protected void makeObjects(Map<String, String> tokens)
			throws BadDataException {
		SectionFooterDataRecord record = new SectionFooterDataRecord();
