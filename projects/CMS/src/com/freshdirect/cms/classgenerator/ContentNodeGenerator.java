...
public class ContentNodeGenerator implements NodeGeneratorI {
    ...
    private final static String PACKAGE_ROOT = "com.freshdirect.cms.classgenerator.gen.";

    /**
     * This flag enables specific optimization, if set to true, memory consumption is greatly reduced, but in the getAttribute call, every time a new Attribute object have to be
     * created. For now, we don't enable this.
     */
    boolean lazy = true;
     ...
     */
    Map<ContentType, Class<?>> contentTypeMap = new HashMap<ContentType, Class<?>>();

    /**
     * Map<EnumAttributeType,CtClass>
     * 
     */
    final Map<EnumAttributeType, CtClass> fieldTypes = new HashMap<EnumAttributeType, CtClass>();

    /**
     * Map<ContentType,Map<String,AttributeDef>>
     */
    final Map<ContentType, Map<String, AttributeDef>> attributeDefMap = new HashMap<ContentType, Map<String, AttributeDef>>();
    ...
    class AttributeMapClassInitializer implements ClassInitializer {
        ...
        Map<String, AttributeDefI> attributeMap;

        public AttributeMapClassInitializer(Map<String, AttributeDefI> attributeMap) {
    ...
    class OrdinalHashClassInitializer implements ClassInitializer {
        ...
        Map<String, Integer> ordinalHash;

        public OrdinalHashClassInitializer(Map<String, Integer> ordinalHash) {
    ...
    class ClassInfo {
        ...
        Class<?> clazz;
        List<ClassInitializer> classInitializers = new ArrayList<ClassInitializer>();
        Map<String, AttributeDefI> attributeMap;
        ...
        public Map<String, AttributeDefI> getAttributeMap() {
            return attributeMap;
        }
        ...
        public void setAttributeMap(Map<String, AttributeDefI> attributeMap) {
            this.attributeMap = attributeMap;
        }
        ...
        public List<ClassInitializer> getClassInitializers() {
    ...
    final class AttributeRecHashComparator implements Comparator<AttributeRec> {

        @Override
        public int compare(AttributeRec a1, AttributeRec a2) {
            return a1.hash == a2.hash ? 0 : a1.hash > a2.hash ? -1 : 1;
        }
    }

    final class AttributeRecNameComparator implements Comparator<AttributeRec> {
    ...
    public static ContentNodeGenerator getGenerator(ContentTypeServiceI service, String prefix) {
        return null;
    }
    ...
    public ContentNodeGenerator(ContentTypeServiceI service, String prefix) {
        ...
        this.packageName = PACKAGE_ROOT + prefix;
        ...
        try {
            ...
            fieldTypes.put(EnumAttributeType.TIME, pool.get("java.util.Date"));
    ...
    private void createTypes() {
        Set<? extends ContentTypeDefI> contentTypeDefinitions = typeService.getContentTypeDefinitions();
        for (Iterator<? extends ContentTypeDefI> iter = contentTypeDefinitions.iterator(); iter.hasNext();) {
            ContentTypeDefI object = iter.next();
            try {
        ...
        for (Iterator<? extends ContentTypeDefI> iter = contentTypeDefinitions.iterator(); iter.hasNext();) {
            ...
            try {
                ...
                for (Iterator<ClassInitializer> citer = info.getClassInitializers().iterator(); citer.hasNext();) {
                    ClassInitializer ci = citer.next();
    ...
    GeneratedContentNode createNodeImpl(ContentType type) {
        Class<?> cls = contentTypeMap.get(type);
        if (cls != null) {
            try {
    ...
    public ContentNodeI createNode(ContentKey key, DraftContext draftContext) {
        GeneratedContentNode node = createNodeImpl(key.getType());
        node.initAttributes();
        node.setKey(key);
    ...
    private ClassInfo createContentType(ContentTypeDefI def) throws CannotCompileException, NotFoundException {
        ...
        Map<String, AttributeDefI> attributeMap = createAttributeMap(def);
...

        {
            ...
            CtMethod method = CtNewMethod.make("public com.freshdirect.cms.ContentTypeDefI getDefinition() {\n" + " return " + implementationClassName
                    + ".__nodeGenerator.getDefinition(\"" + def.getType().getName() + "\");\n" + " }", class1);
            class1.addMethod(method);
        }

        {
            CtMethod method = CtNewMethod.make("public void setAttributeDefs(java.util.Map defs) { \n" + " " + implementationClassName + ".__attributeDefs = defs; " + "}", class1);
            class1.addMethod(method);
        }
        {
            CtMethod method = CtNewMethod.make("public java.util.Map getAttributeDefs() { \n" + "  return " + implementationClassName + ".__attributeDefs; " + "}", class1);
            class1.addMethod(method);
        }
        {
            CtMethod method = CtNewMethod.make("public void setContentNodeGenerator(" + NodeGeneratorI.class.getName() + " value) { \n" + " if ( " + implementationClassName
                    + ".__nodeGenerator != null) { throw new RuntimeException(\"Node generator is already set!\"); }\n" + " " + implementationClassName
                    + ".__nodeGenerator = value; " + "}", class1);
            class1.addMethod(method);
        }
...

        {
            ...
            if (hasAttributes) {
                getAttribute.append(" int hc = name.hashCode(); \n" + " switch (hc) { \n");
                getAttributeValue.append(" int hc = name.hashCode(); \n" + " switch (hc) { \n");
                setAttributeValue.append(" int hc = name.hashCode(); \n" + " switch (hc) { \n");
            }

            StringBuffer copy = new StringBuffer();
            copy.append("public com.freshdirect.cms.ContentNodeI copy() {\n ").append(implementationClassName).append(" result = new ").append(implementationClassName)
            ...
            for (Iterator<String> iter = attributeMap.keySet().iterator(); iter.hasNext();) {
                ...
                AttributeDefI attributeDef = attributeMap.get(name);
                ...
                if (!lazy) {
                    initAttributes.append("  this.").append(attributeFieldName).append(" = new ").append(getImplementationClassName(def.getType(), attributeDef)).append("();\n");
                ...
                if (UNSAFE) {
                    ...
                    setAttributeValue.append(" if (\"").append(name).append("\".equals(name)) { \n").append("   ")
                            .append(getSetStatement(attributeDef, fieldName, fieldType) + " \n    return true; \n}");
                ...
                if (attributeDef instanceof RelationshipDefI) {
                    ...
                    if (r.isNavigable()) {
                        if (EnumCardinality.ONE == r.getCardinality()) {
                            ...
                            getChildKeys.append(" if (").append(fieldName).append("!=null) { \n").append("  result.addAll((java.util.Collection)").append(fieldName)
                                    .append("); \n }");
                        }
                    }
                }
            ...
            /*
             * if (debug) { getAttributeMap.append(" System.out.println(\""+def.getType()+ " - getAttributeMap : \"+result);");
        ...
        for (int i = start; i < end; i++) {
            if (recs[i].name.length() > charPos) {
                ...
                if (currentCharacter != lastCharacter) {
                    ...
                    prevEnd = i;
                    buffer.append(padding).append("  if (ch").append(charPos).append(" == '").append(currentCharacter).append("') { \n");
    ...
    private void createHashMapMethod(ContentTypeDefI def, CtClass class1, ClassInfo ci) throws CannotCompileException, NotFoundException {
        ...
        Map<String, Integer> ordinalhash = new HashMap<String, Integer>();

        StringBuffer buffer = new StringBuffer();
        buffer.append("public final com.freshdirect.cms.AttributeI getAttribute(String name) { \n" + " java.lang.Integer hash = (Integer) __ordinalhash.get(name);\n"
                + " if (hash==null) { return null; }\n" + " int ihash = hash.intValue(); \n" + " switch (ihash) { \n");
        int i = 0;
        for (Iterator<String> iter = ci.attributeMap.keySet().iterator(); iter.hasNext();) {
    ...
    private String createBinarySearchMethod(ContentTypeDefI def, Map<String, AttributeDefI> attributeMap) {
        StringBuffer buffer = new StringBuffer();
        buffer.append("public final com.freshdirect.cms.AttributeI getAttribute(String name) { \n" + " if (name==null) { return null; } \n");
    ...
    private AttributeRec[] createSortedAttributes(Map<String, AttributeDefI> attributeMap, Comparator<AttributeRec> comparator) {
        AttributeRec[] recs = new AttributeRec[attributeMap.size()];
        int i = 0;
        for (Iterator<String> iter = attributeMap.keySet().iterator(); iter.hasNext();) {
            String key = iter.next();
            AttributeRec a = new AttributeRec(key, attributeMap.get(key));
            recs[i] = a;
            i++;
        }
    ...
    private void createField(ContentType contentType, CtClass class1, CtClass type, String fieldName, AttributeDefI attributeDef) throws CannotCompileException, NotFoundException {
...

        {
            CtMethod method = CtNewMethod
                    .make("public java.lang.Object getAttributeValue_" + attributeDef.getName() + "() { " + " return " + getReturnsAsObject(attributeDef) + " }", class1);
            class1.addMethod(method);
        }
        {
            ...
            if (debug) {
                buf.append(" if (value!=null) { \n" + "   System.out.println(\"" + contentType.getName() + "." + attributeDef.getName() + "(" + type.getName()
                        + "):=\"+value + \"(\"+value.getClass().getName()+\")[\"+getKey()+\"]\");\n" + " } ");
            }
    ...
    private String getSetStatement(AttributeDefI attributeDef, String fieldName, CtClass fieldType) throws NotFoundException {
        ...
        if (attributeDef instanceof BidirectionalRelationshipDefI) {
            BidirectionalRelationshipDefI b = (BidirectionalRelationshipDefI) attributeDef;
            return "if (__copy) { \n" + fieldName + " = " + getSetStatement(fieldType) + ";\n } else {\n __nodeGenerator.getReferenceHandler(key.getType(), \""
                    + attributeDef.getName() + "\").addRelation(key, (com.freshdirect.cms.ContentKey) value); }";
        } else {
    ...
    private CtClass createAttributeClass(ContentType type, CtClass objectClass, AttributeDefI attributeDef, CtClass fieldType) throws NotFoundException, CannotCompileException {
        ...
        {
            CtMethod method = CtNewMethod.make("public com.freshdirect.cms.AttributeDefI getDefinition() { return node.getAttributeDef(\"" + attributeDef.getName() + "\"); }",
                    atClass);
            atClass.addMethod(method);
        }
        {
            atClass.addMethod(CtNewMethod.make(
                    "public String toString() { return \"" + type.getName() + ':' + attributeDef.getName() + "=\"+getValue() + \"(" + fieldType.getSimpleName() + ")\";}",
                    atClass));
        }
        {
            atClass.addMethod(CtNewMethod
                    .make("public boolean equals(Object object) { \n" + "  return (object instanceof " + name + ") && (node == ((" + name + ")object).node);\n }", atClass));
        }
    ...
    private String getReturnsAsObject(AttributeDefI attributeDef) {
        if (attributeDef instanceof BackReferenceDefI) {
            ...
            return "__copy ? " + getFieldName(attributeDef) + " : __nodeGenerator.getReferenceHandler(com.freshdirect.cms.ContentType.get(\""
                    + b.getMainRelationship().getSourceType().getName() + "\"), \"" + b.getMainRelationship().getName() + "\").getInverseReference(key);";
        }
        if (attributeDef instanceof BidirectionalRelationshipDefI) {
            BidirectionalRelationshipDefI b = (BidirectionalRelationshipDefI) attributeDef;
    ...
    private CtClass getType(AttributeDefI attributeDef) throws NotFoundException {
        CtClass type = fieldTypes.get(attributeDef.getAttributeType());
        if (type == null) {
            if (attributeDef instanceof RelationshipDefI) {
            ...
            if (attributeDef instanceof EnumDefI) {
                ...
                CtClass enumType = fieldTypes.get(ed.getValueType());
                if (enumType != null) {
                    return enumType;
                }
            }
