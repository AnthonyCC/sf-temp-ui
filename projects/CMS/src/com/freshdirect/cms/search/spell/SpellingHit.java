...
public class SpellingHit implements Serializable, Comparable<SpellingHit> {
	...
	public static SpellingHit join(List<SpellingHit> hits) {
		...
		SpellingHit hit = new SpellingHit(Term.join(phrase, Term.DEFAULT_SEPARATOR), 
				Term.join(spellingMatch, Term.DEFAULT_SEPARATOR), distance);
		return hit;
	}
	...
	public SpellingHit(String phrase, int distance) {
		...
		this.score = Math.max(0.0f, (double) (phrase.length() - distance)) / (double) phrase.length();
	}
