    ...
    <tmpl:put name='content' direct='true'>
        ...
        <fd:Material id='material' sapId='<%= request.getParameter("sapId") %>'>
            ...
            <table>
                ...
                <tr><td class="field_title">Label Name</td><td><input type=text size=60 name='<%= FormElementNameHelper.getFormElementName(material, EnumAttributeName.LABEL_NAME.getName()) %>' value='<%= material.getAttributes().getLabelName() %>'></td></tr>
            ...
            <table cellspacing=2 cellpadding=2>
                ...
                <tr><td class="field_title">Eligible for Perishable Only Promotion</td><td><input type='checkbox' value='true' name='<%= FormElementNameHelper.getFormElementName(material, EnumAttributeName.CUST_PROMO.getName()) %>' <%= material.getAttributes().isCustPromo()?"CHECKED":"" %>></td><tr>
            </table>

            <!-- bottle deposit amount -->
            <table cellspacing=2 cellpadding=2>
                <tr><td class="field_title">Deposit amount</td><td><input size='3' name='<%= FormElementNameHelper.getFormElementName(material, EnumAttributeName.DEPOSIT_AMOUNT.getName()) %>' value='<%= material.getAttributes().getDepositAmount() %>'>
            ...
            <!-- restrictions -->
			<table cellspacing=2 cellpadding=2>
                <tr><td class="field_title">Restrictions</td><td><input size='12' name='<%= FormElementNameHelper.getFormElementName(material, EnumAttributeName.RESTRICTIONS.getName()) %>' value='<%= material.getAttributes().getRestrictions() %>'>
                (comma separated list of additional delivery restriction reason codes, eg. "TKG,VAL,EAS").
                </td><tr>
            </table>
			<table cellspacing=2 cellpadding=2>
                <tr><td class="field_title">Advance Order</td><td><input type='checkbox' name='<%= FormElementNameHelper.getFormElementName(material, EnumAttributeName.ADVANCE_ORDER_FLAG.getName()) %>' <%= (material.getAttributes().isAdvanceOrderFlag())?"CHECKED":"" %>>
            ...
            <table cellspacing=2 cellpadding=2>
                ...
                <tr>
                    ...
                    <td><input type=text size=40 name='<%= FormElementNameHelper.getFormElementName(salesUnit, EnumAttributeName.DESCRIPTION.getName()) %>' value='<%= salesUnit.getAttributes().getDescription() %>'></td>
                    <td align=left><input type=radio name='selected' value='<%= FormElementNameHelper.getFormElementName(salesUnit, EnumAttributeName.SELECTED.getName()) %>' <%= (salesUnit.getAttributes().isSelected())?"CHECKED":"" %>>
                    <%
                     if (idx.intValue()==0) { %>
            ...
            <% ErpsAttributes chAttr = charac.getAttributes(); %>
                ...
                <table>
                    ...
                    <tr><td class="field_title">Description</td><td class="field"><%= chAttr.getDescription() %></td></tr>
                    <tr><td class="field_title">Under Label</td><td class="field"><%= chAttr.getUnderLabel() %></td></tr>
                    <tr><td class="field_title">Priority</td><td class="field"><%= chAttr.getPriority() != 0 ? String.valueOf(chAttr.getPriority()) : "" %></td></tr>
                    <tr><td class="field_title">Optional</td><td class="field"><%= chAttr.isOptional()?"yes":"no" %></td></tr>
                    <tr><td class="field_title">Display Format</td><td class="field"><%= chAttr.getDisplayFormat() %></td></tr>
                    <tr><td colspan=2>
                    ...
                    <table cellspacing=2 cellpadding=2>
                        ...
                        <logic:iterate id="charValue" collection="<%= cvs %>" type="com.freshdirect.erp.model.ErpCharacteristicValueModel">
				            ...
				            <% ErpsAttributes chValAttr = charac.getAttributes(); %>
                            <tr>
