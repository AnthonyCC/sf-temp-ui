...
public class ErpEWalletSessionBean extends SessionBeanSupport {
	...
	public int insertEWalletTxnNotify(ErpEWalletTxNotifyModel eWallettxtNotify) throws RemoteException{
		Connection conn = null;
		int row=0;
		try{
			conn = getConnection();
			row=ErpEWalletTxNotifyDAO.insertEWalletTxnNotify(conn, eWallettxtNotify);
		}catch(SQLException sqle)
		{
			LOGGER.error("Unable to load EWallet Session Bean" , sqle);
			throw new EJBException(sqle);
		}finally {
			try {
				if (conn != null)
	...
	public int updateEWalletTxnNotify(ErpEWalletTxNotifyModel eWallettxtNotify) throws RemoteException{
		Connection conn = null;
		int row=0;
		try{
			conn = getConnection();
			row=ErpEWalletTxNotifyDAO.updateEWalletTxnNotify(conn, eWallettxtNotify);
		}catch(SQLException sqle)
		{
			LOGGER.error("Unable to load EWallet Session Bean" , sqle);
			throw new EJBException(sqle);
	 ...
	 * @throws RemoteException
	 */
	public void updateEWalletTxnNotifyStatus(String orderId, String status) throws RemoteException{
		Connection conn = null;
		try{
			conn = getConnection();
			ErpEWalletTxNotifyDAO.updateEWalletTxnNotifyStatus(conn, orderId, status);
		}catch(SQLException sqle)
		{
			LOGGER.error("Unable to load EWallet Session Bean" , sqle);
			throw new EJBException(sqle);
