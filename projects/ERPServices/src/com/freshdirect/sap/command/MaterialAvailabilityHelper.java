...
//		this.bapi.setPlant( /*SapProperties.getPlant()*/order.getPlant() );
		this.bapi.setPlant( /*SapProperties.getPlant()*/this.orderLine.getPickingPlantId() );
//		this.bapi.setStgeLoc( /*SapProperties.getStgeLoc() */order.getPlant());
		this.bapi.setStgeLoc( /*SapProperties.getStgeLoc()*/this.orderLine.getPickingPlantId() );
	...
	protected void process(BapiMaterialAvailability bapi) {
		...
		for (int i=0; i<size; i++) {
			...
			inventoryEntries.add(
				new ErpInventoryEntryModel( new java.sql.Date( confDate.getTime() ), confQty,this.orderLine.getPickingPlantId()/*order.getPlant()*/ )
			);
		}
