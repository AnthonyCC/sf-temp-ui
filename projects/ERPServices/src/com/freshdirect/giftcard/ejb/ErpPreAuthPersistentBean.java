...
public class ErpPreAuthPersistentBean extends ErpGiftCardTranPersistentBean {
	
	private ErpPreAuthGiftCardModel model;
	
	/** Creates new ErpAuthorizationPersistentBean */
    public ErpPreAuthPersistentBean() {
		super();
		this.model = new ErpPreAuthGiftCardModel();
    }
    
    public ErpPreAuthPersistentBean(PrimaryKey pk){
    	this();
    	this.setPK(pk);
    }
	
	/** Load constructor. */
	public ErpPreAuthPersistentBean(PrimaryKey pk, Connection conn) throws SQLException {
		this();
		this.setPK(pk);
		load(conn);
	}
	 ...
	 */
	public ErpPreAuthPersistentBean(ErpPreAuthGiftCardModel model) {
		this();
	...
	public void setFromModel(ModelI model) {
		this.model = (ErpPreAuthGiftCardModel)model;
		this.setModified();
	}
	...
	public static List findByParent(Connection conn, PrimaryKey parentPK) throws SQLException {
		...
		ps.setString(2, EnumTransactionType.PREAUTH_GIFTCARD.getCode());
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			ErpPreAuthPersistentBean bean = new ErpPreAuthPersistentBean(new PrimaryKey(rs.getString(1)), conn);
	...
	public PrimaryKey create(Connection conn) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("INSERT INTO CUST.GIFT_CARD_TRANS (ID, SALESACTION_ID, TRAN_TYPE, TRAN_STATUS, AUTH_CODE, ERROR_MSG, TRANS_AMOUNT, CERTIFICATE_NUM, CREATE_DATE_TIME, REFERENCE_ID, POST_DATE_TIME) values (?,?,?,?,?,?,?,?,SYSDATE,?,SYSDATE)");
	...
	public void load(Connection conn) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("SELECT TRAN_TYPE, TRAN_STATUS, AUTH_CODE, TRANS_AMOUNT, ERROR_MSG, CERTIFICATE_NUM, REFERENCE_ID, POST_DATE_TIME, CREATE_DATE_TIME FROM CUST.GIFT_CARD_TRANS WHERE SALESACTION_ID = ?");
