...
public class ErpGenerateInvoiceCommand {
	...
	private double fdwRestocking = 0;
	...
	private ErpInvoiceLineModel createNewInvoiceLine(
		...
		if (returnLine.isRestockingOnly()) {
			...
			} else if(ErpAffiliate.getEnum(ErpAffiliate.CODE_FDW).equals(orderLine.getAffiliate())){
				fdwRestocking += returnPrice * ALCOHOL_RESTOCKING_PERC;
			} else {
...
//		double tax = fdProduct.isTaxable() ? invoiceLine.getPrice() * orderLine.getTaxRate() : 0;
	...
	private ErpInvoiceModel makeNewInvoice(ErpAbstractOrderModel order, ErpInvoiceModel oldInvoice, List<ErpInvoiceLineModel> invoiceLines, List charges) {
		...
		if(this.fdwRestocking > 0) {
			newCharges.add(new ErpChargeLineModel(EnumChargeType.FDW_RESTOCKING_FEE, "", this.fdwRestocking, null, 0.0, null));
			total += this.fdwRestocking;
		}
