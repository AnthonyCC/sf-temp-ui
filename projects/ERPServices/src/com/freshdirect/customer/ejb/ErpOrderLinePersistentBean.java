...
public class ErpOrderLinePersistentBean extends ErpReadOnlyPersistentBean {
	...
	public static List findByParent(Connection conn, PrimaryKey parentPK) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("SELECT ID, ORDERLINE_NUMBER, SKU_CODE, VERSION, QUANTITY, SALES_UNIT, CONFIGURATION, PROMOTION_TYPE, PROMOTION_AMT, PROMOTION_CAMPAIGN, DESCRIPTION, CONFIGURATION_DESC, DEPARTMENT_DESC, MATERIAL_NUMBER, PRICE, PERISHABLE, TAX_RATE, DEPOSIT_VALUE, ALCOHOL, AFFILIATE, CARTLINE_ID, RECIPE_SOURCE_ID, REQUEST_NOTIFICATION, RATING,BASE_PRICE,BASE_PRICE_UNIT,DISCOUNT_AMT, VARIANT_ID, SAVINGS_ID,PRICING_ZONE_ID,GROUP_ID,GROUP_VERSION,GROUP_QTY,SUSTAINABILITY_RATING,SKU_LIMIT, MAX_PERCENTAGE_DISCOUNT FROM CUST.ORDERLINE WHERE SALESACTION_ID=? ORDER BY ORDERLINE_NUMBER");
	...
	private final static String INSERT_QUERY =
		...
		+ " TAX_RATE, DEPOSIT_VALUE, ALCOHOL, AFFILIATE, CARTLINE_ID, RECIPE_SOURCE_ID, REQUEST_NOTIFICATION,RATING,BASE_PRICE,BASE_PRICE_UNIT,DISCOUNT_AMT,VARIANT_ID,SAVINGS_ID,PRICING_ZONE_ID,GROUP_ID,GROUP_VERSION,GROUP_QTY,SUSTAINABILITY_RATING, SKU_LIMIT, MAX_PERCENTAGE_DISCOUNT)"
		+ " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?)";
	...
	public PrimaryKey create(Connection conn) throws SQLException {
		...
		if (d!=null) {		
			...
			ps.setDouble(37, d.getMaxPercentageDiscount());
		} else {
	...
	private void loadFromResultSet(ResultSet rs) throws SQLException {
		...
		if(!rs.wasNull()){
			...
			discount.setMaxPercentageDiscount(rs.getDouble("MAX_PERCENTAGE_DISCOUNT"));
