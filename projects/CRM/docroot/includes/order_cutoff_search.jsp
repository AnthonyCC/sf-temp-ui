						...
						<td colspan="2" align="center">
							<table width="80%" cellpadding="0" cellspacing="2">
								...
								<logic:iterate collection="<%= cutoffReport%>" id="cReport" type="com.freshdirect.fdstore.customer.FDCutoffTimeInfo">
									<tr>
										<td><%=null !=cReport.getCutoffTime()?CCFormatter.formatDeliveryTime(cReport.getCutoffTime()):""%></td>
										<td><!-- a href='javascript:changeAction(document.forms["searchByCutoff"], "order_quicksearch_results.jsp?status=<%=cReport.getStatus().getStatusCode()%>&cutoffTime=<%=cReport.getCutoffTime()%>&day=<%=request.getParameter("day")%>&month=<%=request.getParameter("month")%>&year=<%=request.getParameter("year")%>");'--><%=cReport.getStatus().getName()%><!-- /a--></td>
