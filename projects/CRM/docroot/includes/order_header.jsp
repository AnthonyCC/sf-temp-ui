	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="sub_nav">
		<tr>
			...
			<% if (isCheckout) { %>
                ...
                <% } %>
			<% } else { %>
			<b><span class="order_step">Build Order: <a href="<%= response.encodeURL("/order/place_order_build.jsp") %>" class="order_step">Search</a> &middot; <a href="<%= response.encodeURL("/order/build_order_browse.jsp") %>" class="order_step">Browse</a><% if ( user != null && user.getIdentity() != null ) { %> &middot; <a href="/order/quickshop/quickshop_index.jsp" class="order_step">QuickShop</a> &middot; <a href="<%= response.encodeURL("/order/build_order_recipes.jsp") %>" class="order_step">Recipes</a><fd:CCLCheck> &middot; <a href="/order/quickshop/all_lists.jsp" class="order_step">Lists</a></fd:CCLCheck><% } %>&nbsp;&nbsp;|&nbsp;&nbsp;</span><%if(hasCustomerCase){%><a href="<%= response.encodeURL("/checkout/checkout_review_items.jsp") %>">Checkout</a><%} else {%><span class="cust_module_content_edit">Case required to checkout</span><% } %></b>
			<% } %>
			</td>
