...
<script language="javascript">
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="sub_nav">
		<tr>
			...
			<% } else { %>
				<b>
					<span class="order_step">Build Order: 
					...
					<% } %>
					<%
						String ecouponsUrl = "/order/build_order_browse.jsp?catId="+FDCouponProperties.getCouponCMSCategory();
					%>
					&middot; <a href="<%= response.encodeURL(ecouponsUrl) %>" class="order_step">Ecoupons</a>
					&nbsp;&nbsp;|&nbsp;&nbsp;</span>
					<%if(hasCustomerCase){%>
						<a href="<%= response.encodeURL("/checkout/checkout_review_items.jsp") %>">Checkout</a>
					<%} else {%>
						<span class="cust_module_content_edit">Case required to checkout</span>
					<% } %>
				</b>
			<% } %>
			</td>
