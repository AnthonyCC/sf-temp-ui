...
We were unable to process the e-Check payment for your order (#<xsl:value-of select="orderNumber"/>), which was delivered on <xsl:call-template name="format-delivery-date"><xsl:with-param name="dateTime" select="deliveryStartTime"/></xsl:call-template>.
Please contact customer service at <xsl:value-of select="customer/customerServiceContact"/>, within the next 24 hours so that we may help you resolve the payment discrepancy.
