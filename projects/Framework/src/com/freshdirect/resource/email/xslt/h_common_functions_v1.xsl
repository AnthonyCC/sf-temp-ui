...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dt='http://xsltsl.org/date-time' version="1.0">
	...
	<xsl:template name="format-delivery-address">
		<xsl:value-of select="order/delivery-info/address/@address1"/><xsl:text> </xsl:text><xsl:if test="string-length('order/delivery-info/address/@apartment') &gt; 0"><xsl:value-of select="order/delivery-info/address/@apartment"/></xsl:if><br/>
	...
	<xsl:template name="format-payment-method">
		<xsl:value-of select="order/payment-method/@name"/><br/>
		<xsl:value-of select="order/payment-method/@brand"/> # <xsl:value-of select="order/payment-method/@account-number"/><br/>
	</xsl:template>

	<!-- DELIVERY ADDRESS TEMPLATE -->
	<xsl:template match="address">
		<xsl:value-of select="@address1"/><xsl:text> </xsl:text><xsl:if test="@apartment"><xsl:value-of select="@apartment"/></xsl:if><br/>
		...
		<br/>
		Phone: <xsl:value-of select="phone/@phone"/> <xsl:if test="phone/@ext != ''"> Ext. <xsl:value-of select="phone/@ext"/></xsl:if><br/>
