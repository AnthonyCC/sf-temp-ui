...
<body bgcolor="#CCCCCC">
	...
	<div style="background-color: #ffffff; border: 1px solid #bbbbbb; margin: 5px; padding: 20px;">
		...
		<div style="margin: 15px 0; font-size: 16px;">
			<span style="font-weight: bold;">So the bad news first:</span> we weren't able to process the e-check payment for your order <xsl:value-of select="orderNumber"/> on <xsl:call-template name="format-delivery-date"><xsl:with-param name="dateTime" select="order/deliveryReservation/startTime"/></xsl:call-template>. The good news: it's usually just an input error so give us a call and we'll figure it out.
			<br /><br />
