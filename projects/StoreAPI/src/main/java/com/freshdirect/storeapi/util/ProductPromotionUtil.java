...
import com.freshdirect.storeapi.content.ProductPromotionData;
...
//		zoneSkusMap =fdProductPromotion.getZoneSkusMap();
						...
						if(null != skusList){
							for (Iterator iterator3 = skusList.iterator(); iterator3.hasNext();) {
								...
								if(null == productModel){
									try {
										productModel = ((SkuModel) ContentFactory.getInstance().getContentNodeByKey(ContentKeyFactory.get(FDContentTypes.SKU, skuCode))).getProductModel();
	...
	public final static Comparator<ProductModel> DEPTFULL_COMPARATOR = new Comparator<ProductModel>() {
        ...
        public int compare(ProductModel p1, ProductModel p2) {
			...
			ProductModel prodModel1 = ((SkuModel) ContentFactory.getInstance().getContentNodeByKey(ContentKeyFactory.get(FDContentTypes.SKU, p1.getDefaultSkuCode()))).getProductModel();
			ProductModel prodModel2 = ((SkuModel) ContentFactory.getInstance().getContentNodeByKey(ContentKeyFactory.get(FDContentTypes.SKU, p2.getDefaultSkuCode()))).getProductModel();
	...
	public static Map<ZoneInfo, List<FDProductPromotionInfo>> formatProductPromotionPreviewInfo(
		...
		if(null !=productPromotionInfoMap){
			for(Iterator itr = productPromotionInfoMap.keySet().iterator();itr.hasNext();){
				...
				List<FDProductPromotionInfo> productPromotionInfoList =productPromotionInfoMap.get(zoneInfo);
				List<FDProductPromotionInfo> productPromotionPreviewInfoList = productPromotionPreviewInfoMap.get(zoneInfo);
