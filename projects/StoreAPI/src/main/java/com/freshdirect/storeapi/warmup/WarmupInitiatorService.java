...
public class WarmupInitiatorService {
    ...
    /**
     * This method gets warmup state via reflection. This throws Exception as non of the current users of this method are interested in the exact exception. Refine
     * this if and when it is needed.
     * 
     * @throws Exception
     *             if something goes wrong
     */
    public Object isWarmupFinished() throws Exception {
        Class<?> warmupServiceClass;
        String className = WARMUP_SERVICE_CLASS_NAME;
        warmupServiceClass = Class.forName(className);
        Object warmupService = warmupServiceClass.getMethod(DEFAULT_SERVICE_METHOD_NAME, null).invoke(warmupServiceClass, null);
        return warmupService.getClass().getMethod(IS_WARMUP_FINISHED_NAME, null).invoke(warmupService, null);
    }
