...
import com.freshdirect.storeapi.spelling.domain.CsongorDistance;
...
import com.freshdirect.storeapi.spelling.domain.SuggestWord;
import com.freshdirect.storeapi.spelling.domain.SuggestWordQueue;
...
public class SpellChecker {
     ...
     * 
     * <p>
     * As the Lucene similarity that is used to fetch the most relevant n-grammed terms is not the same as the edit distance strategy used to calculate the best matching
     * spell-checked word from the hits that Lucene found, one usually has to retrieve a couple of numSug's in order to get the true best match.
     * 
     * <p>
     * I.e. if numSug == 1, don't count on that suggestion being the best one. Thus, you should set this value to <b>at least</b> 5 for a good suggestion.
     * 
     * @param indexPath
     * 
     * @param phrase
     ...
     * @throws IOException
     *             if the underlying index throws an {@link IOException}
     * @return String[] the sorted list of the suggest words with these 2 criteria: first criteria: the edit distance, second criteria (only if restricted mode): the popularity of
     *         the suggest words in the field of the user index
     */
    public List<SpellingHit> suggestSimilar(String indexPath, String phrase, int numSug, String field) throws IOException {
        ...
        for (Document document : searchHits) {
            ...
            // edit distance
            sugWord.distance = stringDistance.getDistance(phrase, sugWord.searchTerm);
