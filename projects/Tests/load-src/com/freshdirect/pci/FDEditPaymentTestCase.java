...
public class FDEditPaymentTestCase extends DbTestCaseSupport {
    ...
    public FDEditPaymentTestCase(String name) throws Exception{
        super(name);
	...
	private ErpPaymentMethodI getNewCCMethod() throws Exception {
        String month = "09";
        String year = "2010";
        String cardType = "MC";
        String accountNumber = "5405222222222226";
...
//	}
	...
	public void testEditPaymentMethod() throws Exception {
		...
		ErpPaymentMethodModel paymentMethod = (ErpPaymentMethodModel)getNewCCMethod();
		ErpPaymentMethodPersistentBean localBean = new ErpPaymentMethodPersistentBean(paymentMethod);
		localBean.setPK(bean.getPK());
		localBean.setParentPK(bean.getParentPK());
		localBean.store(conn);
		//Load it again to make sure the update has happened.
		localBean.load(conn);
		paymentMethod = (ErpCreditCardModel)localBean.getModel();
		assertEquals("5405222222222226", paymentMethod.getAccountNumber());
	}
	
	
	public static Test suite() {
		return new TestSuite(FDEditPaymentTestCase.class);
	}
