...
public class GeneralDriver extends BaseModuleDriver {
	...
	throws InterruptedException, FindFailed {
		if (uiDriver.isDisplayed("btnlogout")) {
			Logout(input, output);
		}else if(!uiDriver.isDisplayed("btnlogout")&&!uiDriver.isDisplayed("btnlogin")){
						...
						if (str1.equalsIgnoreCase("merge")) {
							...
							if (we.isSelected()) {
								RESULT.passed("Merge radio btn should be selected", 
										"Should select Merge radio btn", "Merge radio button selected");
	...
	throws InterruptedException, TimeoutException, FindFailed {
		try {	
			...
			if(input.get("ServiceType").equalsIgnoreCase("OFFICE"))
			{
				uiDriver.click("btnlogout");
				uiDriver.wait.until(ExpectedConditions.visibilityOfElementLocated(By.className(objMap.getLocator("btnlogin"))));
				uiDriver.FD_login(input.get("Email"), input.get("PassWord1"));
				// wait for logout button
				uiDriver.wait.until(ExpectedConditions.visibilityOfElementLocated(By.className(objMap.getLocator("btnlogout"))));
				if (uiDriver.isDisplayed("btnlogout")) {
					RESULT.passed("Login", 
				...
				if(url.contains("COS.jsp"))
				{
					RESULT.passed("Signup", 
							"User should be able to sign up successfully as a corporate customer",
					"User has sign up successfully as a corporate customer");
				}
				else
				{
					RESULT.warning("Signup", 
							"User should be able to sign up successfully as a corporate customer",
					"Unable to sign up successfully as a corporate customer");
				}
			}
		} catch (Exception e) {
			//e.printStackTrace();
	...
	public void PasswordValidation(DataRow input, DataRow output){
		try {
			...
			e.printStackTrace();
			RESULT.failed("SignUp Page", "Error in SignUp Page", "Error occurred in SignUp Page");
		}
		try {
			...
			e.printStackTrace();
			RESULT.warning("Acc pref Page", "Error in Acc pref Page", "Error occurred in Acc pref Page");
		}
	}
