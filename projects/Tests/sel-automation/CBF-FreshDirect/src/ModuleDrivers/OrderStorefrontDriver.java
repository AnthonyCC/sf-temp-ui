...
//				SleepUtils.getInstance().sleep(TimeSlab.HIGH);
			...
			if(webDriver.findElements(By.xpath(objMap.getLocator("chkageVerificationAlcohol"))).size() > 0)
			{
				...
				WebElement element = uiDriver.getwebDriverLocator(objMap.getLocator("btnchangeDeliveryAddress"));
...
//			WebUIDriver.selenium.waitForPageToLoad(uiDriver.PageLoadTime);
			if (webDriver.findElements(By.xpath(objMap.getLocator("btnChangeTimeSlot"))).size() > 0) {
				if (webDriver.findElements(By.linkText(objMap.getLocator("btnselectTime"))).size() > 0) {
					uiDriver.wait.until(ExpectedConditions.presenceOfElementLocated(By.linkText("btnselectTime")));  
...
//						return;
					...
					String DeliveryDay = webDriver.findElement(By.xpath(objMap.getLocator("strdeliveryDay"))).getText();
					System.out.println("Text:" + DeliveryDay);
					String TimeSlot = webDriver.findElement(By.xpath(objMap.getLocator("strdeliveryTime"))).getText();
					System.out.println("Text:" + TimeSlot);
					TimeSlot = TimeSlot.replaceAll(" ", "");
					String TimeSlot1 = TimeSlot.split("-")[0];
					String TimeSlot2 = TimeSlot.split("-")[1];
					...
					if (TimeSlot.toLowerCase().contains(input.get("TimeSlot").toLowerCase())&& 
						...
						"Given  Time Slot allready Selected");
					} else {
	...
	throws FindFailed, InterruptedException {
		try {
			if (webDriver.findElements(By.xpath(objMap.getLocator("btnchangePaymnt"))).size()>0)
			{
				...
				SleepUtils.getInstance().sleep(TimeSlab.YIELD);
				RESULT.done("Change Payment button","Click on change button",
				...
				try{
					...
					if(uiDriver.getwebDriverLocator(objMap.getLocator("btnchangePaymnt")).isDisplayed())
						RESULT.passed("Change Payment button",
	...
	throws FindFailed, InterruptedException {
		...
		if (webDriver.findElements(By.xpath(objMap.getLocator("strorderNumber"))).size() > 0) {
			...
			RESULT.failed("Order Placed", "Order should be submitted","Order is not submitted.");
		}
		}catch (Exception e) {
			 e.printStackTrace();
		}
	...
	InterruptedException {
		...
		if(webDriver.findElements(By.linkText(objMap.getLocator("lnkdeliveryInfoLink"))).size()>0 && webDriver.findElements(By.className(objMap.getLocator("btnlogout"))).size()>0)
		{
			...
			webDriver.get("http://dev9.freshdirect.com/your_account/delivery_info_avail_slots.jsp");
		...
		else if(webDriver.findElements(By.linkText(objMap.getLocator("lnkdeliveryInfoLink"))).size()>0 && webDriver.findElements(By.className(objMap.getLocator("btnlogin"))).size()>0)
		{
			uiDriver.click("lnkdeliveryInfoLink"); 					
			try{	
				webDriver.get("http://dev9.freshdirect.com/help/delivery_info_check_slots.jsp");
