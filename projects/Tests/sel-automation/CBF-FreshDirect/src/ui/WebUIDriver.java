				...
				if (flag == 0) {
					...
					for (WebElement ul_ite : ul_list) {
						...
						for (WebElement li_ite : li_list) {
							...
							// product_text=li_ite.findElement(By.xpath("span/label")).getText();
							// System.out.println(product_text);
							if (product_text.contains(Timeframe)) {
								// click
								li_ite.findElement(By.xpath("span/label/input")).click();
								flag = 2;
								// webDriver.findElement(By.className("qs-popup-close-icon")).click();
								// Thread.sleep(5000);
		...
		if (m.find()) {
			...
			if (email.contains("`") || email.contains("~")
				...
				RESULT.failed("Email id validation",
						"Email id should match to the pattern",
				"Email id should not contain special characters");
			} else if (email.split(" ").length > 1) {
				RESULT.failed("Email id validation",
						"Email id should match to the pattern",
				"Email id should not contain blank or white space");
			} else if (spltemail.length > 2) {
				RESULT.failed("Email id validation",
						"Email id should match to the pattern",
				"Email id should not contain more than one '@' character");
			} else if (spltemail.length < 2) {
				RESULT.failed("Email id validation",
						"Email id should match to the pattern",
				"Email id should exact one '@' symbol");
			} else {
...
//			uiDriver.selenium.waitForPageToLoad(PageLoadTime);
	...
	*Function Name: FD_newCustomerCRM
	...
	public void FD_newCustomerCRM(String CustomerType, String Zip,
		...
		uiDriver.setValue("txtemail_CRM", EmailCRM);
