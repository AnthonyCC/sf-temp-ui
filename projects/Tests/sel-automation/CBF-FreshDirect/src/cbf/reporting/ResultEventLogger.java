/******************************************************************************
$Id : ResultEventLogger.java 9/8/2014 1:21:56 PM
...
public class ResultEventLogger implements ResultReporter {
	...
	public static Integer id, testCaseID, iterationID, testStepID, componentID;
	...
	public ResultEventLogger(Map params) {
		...
		if(filePath.equals("")){
			filePath=ResourcePaths.singleton.getRunResource("Events.xml","");
		}
	}
	...
	public void close() {
		...
		/*
		 * Object[] temp = {"'#TestsExecuted#'","'15'"}; Map<Object,Object> obj
		 * = Utils.toMap(temp); obj.put("'#TestsFailed#'","'3'");
		 * obj.put("'#Totalduration#'","'4:30'"); try { replaceData(obj); }
	...
	public void open(Map headers) {
		...
		if (!isExists) {
			try {
				...
				logger.handleError(
						"Error in making a new report file from template", targetFile, e);
			}
		}
		...
		try {
			...
			case TESTCASE:
				...
				if (eventType.equals("FINISH")) {
					...
					String targetFile= ResourcePaths.singleton.getRunResource("ResultsViewer.lnk", "");
					
					String templateFile = ResourcePaths.singleton.getFrameworkResource("Misc_ResultsViewer", "ResultsViewer.hta");
					CreateShortCut(templateFile, targetFile);//.substring(0, targetFile.length()-1));
				}
				break;
			case ITERATION:
				if (eventType.equals("START")) {
	...
	private Map detaildata(Object eventData) {
		...
		mtemp.remove("delete");
		screendump=false;
		if(!mtemp.get("'name'").equals("''") && mtemp.get("'screenDump'").equals("'true'")){
			screendump=true;
		}
		return mtemp;
	}
	...
	private Map Step2Map(Object entityDetails) {
		...
		obj
				.put("'failTestIfUnexpected'", "'" + ts.failTestIfUnexpected()
						+ "'");
	...
	private void CreateShortCut(String sourcePath, String targetPath)
		...
		if (FileUtils.fileExists(sourcePath)) {
			String script = "Set sh = CreateObject(\"WScript.Shell\")"
					...
					+ "\nshortcut.Save";
			String tempFile = targetPath.substring(0, targetPath
					.lastIndexOf("/"))
					+ "/temp.vbs";
