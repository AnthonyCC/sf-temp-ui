...
public class AsyncRefreshLruCacheTest extends TestCase {

	private AsyncDBRefreshLruCache<String, Object> cache;

	private AsyncDBRefreshLruCache<String, Object> buildCache(String name, int num) {
		return new AsyncDBRefreshLruCache<String, Object>(name, num);
	}
	@Test
	public void testReadWriteEhcache() {
		cache = buildCache("testNewLruCache1", 10);
	...
	public void testCapacity() {
		cache = buildCache("testNewLruCache2", 10);
		assertEquals(cache.size(), 0);
	...
	public void testRemove() {
		cache = buildCache("testNewLruCache3", 10);
		assertEquals(cache.size(), 0);
	...
	public void testUpdate() {
		cache = buildCache("testNewLruCache4", 10);
		assertEquals(cache.size(), 0);
