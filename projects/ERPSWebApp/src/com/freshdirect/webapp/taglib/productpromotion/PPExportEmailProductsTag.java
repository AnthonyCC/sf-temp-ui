	...
	private int populateBuffer(StringBuffer buffer, int i, String zoneId,
		...
		if(null != skusList){
			for (Iterator iterator3 = skusList.iterator(); iterator3.hasNext();) {
				...
				try {
					ProductModel prodModel = ((SkuModel) ContentFactory.getInstance().getContentNodeByKey(ContentKeyFactory.get(FDContentTypes.SKU, promotionSkuModel.getSkuCode()))).getProductModel();
