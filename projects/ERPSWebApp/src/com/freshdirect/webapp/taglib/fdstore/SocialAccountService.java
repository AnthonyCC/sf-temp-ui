...
public class SocialAccountService implements AccountService {
    ...
    public String login(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (socialUserProfile != null) {
            ...
            if (isUserIdExistForUserToken(userToken)) {
                ...
                /*
                 * on Account Setting page Check if the just authenticated social email has been associated with the current logged in FD user
                 */
                if (sessionUserId != null && !socialUserId.equalsIgnoreCase(sessionUserId) && (sessionUserLevel == FDUserI.SIGNED_IN)) {
                ...
                /*
                 * APPBUG-3955 on Account Setting page Check if the just authenticated social email has been associated with the current logged in FD user
                 */
                if (sessionUserId != null && socialUserId.equalsIgnoreCase(sessionUserId) && (sessionUserLevel == FDUserI.SIGNED_IN)) {
                ...
                /*
                 * The socialUserId has been linked with FD account. Auto Login.
                 */
                 ...
                 */
                if (sessionUserId != null && !socialUserId.equalsIgnoreCase(sessionUserId) && (sessionUserLevel == FDUserI.RECOGNIZED)) {
                    return signinUnmatched;
                }

                /*
                 * check if there exist a matching FD account if exist, 1) link the just authenticated social email with the FD account, 2) auto login if not,
                 */
                ...
                if (isUserEmailAlreadyExist(socialUserId)) {
                    ...
                    try {
                        ...
                        ExternalAccountManager.linkUserTokenToUserId(customerId, socialUserProfile.get("email"), socialUserProfile.get("userToken"),
                                socialUserProfile.get("identityToken"), socialUserProfile.get("provider"), socialUserProfile.get("displayName"),
                                socialUserProfile.get("preferredUsername"), socialUserProfile.get("email"), socialUserProfile.get("emailVerified"));
    ...
    @Override
    public String register(FDSessionUser user, PageContext pageContext, ActionResult actionResult, int registrationType) throws Exception {
        ...
        /*
         * if(pageContext.getSession().getAttribute("REFERRALNAME") != null ) { if(!"done".equals(request.getParameter("submission"))) { actionResult.addError(new
         * ActionError("Dummy","Dummy")); return true; } }
         */
            ...
            if (FDStoreProperties.isExtoleRafEnabled() && pageContext.getSession().getAttribute("CLICKID") != null) {
                try {
                    ...
                    LOGGER.debug("Adding referral record for CID:" + user.getIdentity().getErpCustomerPK() + "-email:" + user.getUserId() + "-reflink:"
                            + (String) pageContext.getSession().getAttribute("CLICKID"));
     ...
     */
    private void createUser(EnumServiceType serviceType, Set<EnumServiceType> availableServices, HttpSession session, HttpServletResponse response) throws FDResourceException {
        // HttpSession session = pageContext.getSession();
        // HttpServletResponse response = (HttpServletResponse) pageContext.getResponse();
