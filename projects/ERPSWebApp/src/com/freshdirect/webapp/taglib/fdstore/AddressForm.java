...
public class AddressForm implements WebFormI { // , AddressName
    ...
    public void populateForm(HttpServletRequest request) {
        ...
        /*
         * if ( homePhone == null || homePhone.trim().equals("") ) { homePhone = request.getParameter("homephone"); }
         */
        ...
        if ("true".equals(request.getParameter(EnumUserInfoName.DLV_UNATTENDED_CONSENT_SEEN.getCode()))) {
            ...
            if (unattendedDeliveryStr != null) {
                if (unattendedDeliveryStr.equals("OPT_IN"))
    ...
    private void validateDeliveryFields(ActionResult result) {
        ...
        result.addError(altContactPhone != null && PhoneNumber.normalize(altContactPhone).startsWith("0"), EnumUserInfoName.DLV_ALT_CONTACT_PHONE.getCode(),
    ...
    private void validateAltDeliveryFields(ActionResult result) {
        ...
        if (EnumDeliverySetting.NEIGHBOR.getDeliveryCode().equalsIgnoreCase(alternateDeliverySetting)) {
            ...
            result.addError(altDlvLastName == null || altDlvLastName.length() < 1, EnumUserInfoName.DLV_ALT_LASTNAME.getCode(), SystemMessageList.MSG_REQUIRED);
            result.addError(altDlvFirstName == null || altDlvFirstName.length() < 1, EnumUserInfoName.DLV_ALT_FIRSTNAME.getCode(), SystemMessageList.MSG_REQUIRED);
            result.addError(altDlvApartment == null || altDlvApartment.length() < 1, EnumUserInfoName.DLV_ALT_APARTMENT.getCode(), SystemMessageList.MSG_REQUIRED);
