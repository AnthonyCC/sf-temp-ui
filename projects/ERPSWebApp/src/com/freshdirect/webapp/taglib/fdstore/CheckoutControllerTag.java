...
public class CheckoutControllerTag extends AbstractControllerTag {
	...
	protected boolean performAction(HttpServletRequest request, ActionResult result) throws JspException {
		...
		try {
			...
			} else if("addPaymentMethod".equalsIgnoreCase(action)) {
	...
	protected void performDeleteDeliveryAddress(HttpServletRequest request, ActionResult result) throws FDResourceException {
	...
	protected String performReserveDeliveryTimeSlot(HttpServletRequest request, ActionResult result) throws Exception {
		setPhoneCharge(request, result);
		return setAndReserveDeliveryTimeSlot(request,result);
	}
	
	
	protected void performSetPaymentMethod(HttpServletRequest request, ActionResult result) throws FDResourceException {
		setPaymentMethod(request, result);
		if ( result.isSuccess() ) {
	...
	protected void performAddAndSetPaymentMethod(HttpServletRequest request, ActionResult result) throws FDResourceException {
		//
		...
		//
		List payMethods = FDCustomerFactory.getErpCustomer(identity).getPaymentMethods();
	...
	protected void performAddPaymentMethod(HttpServletRequest request, ActionResult result) throws FDResourceException {
	...
	protected void performDeletePaymentMethod(HttpServletRequest request, ActionResult result) throws FDResourceException {
	...
	protected void performSetDeliveryAddressAndPayment(HttpServletRequest request, ActionResult result) throws FDResourceException, JspException {
	...
	protected void performEditAndSetDeliveryAddress(HttpServletRequest request, ActionResult result) throws FDResourceException {
		...
		boolean foundFraud = AddressUtil.updateShipToAddress(request, result, this.getUser(), addressId, erpAddress);
		
		if (!result.isSuccess()) return;
		
		ErpAddressModel thisAddress = FDCustomerManager.getShipToAddress(getIdentity(), addressId);
		
		String zoneId = zoneInfo.getZoneCode();
		if (zoneId!=null && zoneId.length()>0) {
			LOGGER.debug("success! adding address to cart & setting defaultShipToAddress.");
			FDCustomerManager.setDefaultShipToAddressPK(getIdentity(),((ErpAddressModel)thisAddress).getPK().getId());
	...
	protected void performSetDeliveryAddress(HttpServletRequest request, ActionResult result) throws FDResourceException, JspException {
		...
		if (addressOrLocation.startsWith("DEPOT_")) { 
			...
			this.setRegularDeliveryAddress(user, result, addressOrLocation, request);
		}
		FDCartModel cart = user.getShoppingCart();
		if(user.getSelectedServiceType() == EnumServiceType.HOME && (user.isDlvPassActive() 
				|| cart.getDeliveryPassCount() > 0) && !(cart.isDlvPromotionApplied())) {
	...
	private void setRegularDeliveryAddress(FDUserI user, ActionResult result, String addressPK, HttpServletRequest request) throws FDResourceException, JspException {
		FDIdentity identity = user.getIdentity();
		
		// locate the shipto address with the specified PK
		ErpAddressModel shippingAddress = FDCustomerManager.getShipToAddress(identity, addressPK);
		...
		//if it is a Hamptons address without the altContactNumber have user edit and provide it.
...
//		 since address looks alright need geocode 
		...
		//
		// if the user opted out or has not seen the unattended delivery notice, simply ignore
		// if he opted in, check if the unattended delivery is available for the zone.
		// if not, set the flag to DO_NOT_USE. This will ensure that Unattended Delivery
		...
		if (EnumUnattendedDeliveryFlag.OPT_IN.equals(shippingAddress.getUnattendedDeliveryFlag())) {
			...
			if (!dlvResponse.isUnattended()) {
				...
				LOGGER.debug("Keeping user unattended delivery instructions: " +
						(shippingAddress.getUnattendedDeliveryInstructions() == null ? 
								"OK" : 
								shippingAddress.getUnattendedDeliveryInstructions()));
			...
			if (dlvResponse.isUnattended()) {
				...
				if ("OPT_IN".equals(request.getParameter(EnumUserInfoName.DLV_UNATTENDED_DELIVERY_OPT.getCode()))) {
					...
					if ("".equals(unattendedInstructions)) unattendedInstructions = null;
		...
		FDCustomerManager.setDefaultShipToAddressPK(identity, shippingAddress.getPK().getId());
	...
	private void setDepotDeliveryLocation(String locationId, HttpServletRequest request, ActionResult result) throws FDResourceException {
		...
		if(depot.isPickup()){
			/*
			 * [segabor] DEAD CODE ALERT: contactNumber never gets non-null value
			 */
			...
			if (contactNumber != null && !"".equals(contactNumber)) {
				...
				if (!contactPhone.equals(infoModel.getOtherPhone())) {
					...
					FDCustomerManager.updateCustomerInfo(AccountActivityUtil.getActionInfo(pageContext.getSession()), infoModel);
				}
			}
		}
...
//			 since address need geocode
	...
	private String setAndReserveDeliveryTimeSlot(HttpServletRequest request, ActionResult result) throws Exception {
	...
	private void setPhoneCharge(HttpServletRequest request, ActionResult result) throws FDResourceException {
	...
	private void setPaymentMethod(HttpServletRequest request, ActionResult result) throws FDResourceException {
	...
	private boolean isAgeVerificationNeeded(String app,HttpServletRequest request) throws FDResourceException{
		FDCartModel cart = getCart();
		return (
			cart.containsAlcohol() 
			&& !"CALLCENTER".equalsIgnoreCase(app)  
			&& !cart.isAgeVerified() 
			&& request.getRequestURI().indexOf("/step_1")==-1);
	}
	private void setPaymentMethod(HttpServletRequest request, ActionResult result, String paymentId, String billingRef, boolean makeGoodOrder, String referencedOrder) throws FDResourceException {
		...
		for (Iterator iterator = paymentMethods.iterator(); iterator.hasNext(); ) {
			...
			if ( ((ErpPaymentMethodModel)item).getPK().getId().equals(paymentId) ) {
				paymentMethod = item;
				break;
			}
		}
		...
		//
		// set default payment method and check for unique billing address, if required
		//
		FDActionInfo info = AccountActivityUtil.getActionInfo(session);
		FDCustomerManager.setDefaultPaymentMethod(info, ((ErpPaymentMethodModel)paymentMethod).getPK());
