...
public class FeaturedProductsUtil
{
    private static final Logger LOG = LoggerFactory.getInstance(FeaturedProductsUtil.class);
		...
		for (Iterator<HLBrandProductAdInfo> iterator = hlBrandAdProductsMeta.iterator(); iterator.hasNext();) {
			...
			if (null != productModel && !productModel.isUnavailable()) {
				...
				try {
					...
					LOG.warn("Exception while populating Featured returned product: ", e);
