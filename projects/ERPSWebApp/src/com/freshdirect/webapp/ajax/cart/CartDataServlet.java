    ...
    private static void process(HttpServletRequest request, HttpServletResponse response, FDUserI user) throws HttpErrorResponse {
        ...
        synchronized (cart) {
            ...
            try {
                ...
                /*
                 * FDCustomerCreditHistoryModel creditHistory = null; try { FDIdentity identity = user.getIdentity(); if ( identity != null ) { creditHistory =
                 * FDCustomerManager.getCreditHistory( identity ); } } catch (FDResourceException e) { LOG.error( "Failed to get credit history for " + userId, e ); } if (
                 * creditHistory == null ) { LOG.error( "Failed to get credit history for " + userId ); } else { double remaining = creditHistory.getRemainingAmount(); if (
                 * remaining > 0.00 ) { cartData.setRemainingCredits( remaining ); } }
                 */
                ...
                for (FDCartLineI cartLine : cartLines) {
                    ...
                    /*
                     * placeholder for marking products as free sample products do the same logic here as view cart
                     */
