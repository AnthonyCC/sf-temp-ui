...
public class AvailabilityService {
	...
	private static final String BELOW_MINIMAL_ORDER = "below_minimal_order";
	private static final String PICKUP_BELOW_MINIMAL_ORDER = "pickup_below_minimal_order";
	private static final String ALCOHOL_BELOW_MINIMAL_ORDER = "alcohol_below_minimal_order";
	private static final String ALCOHOL_PICKUP_BELOW_MINIMAL_ORDER = "alcohol_pickup_below_minimal_order";
	...
	public Map<String, String> populateWarningMessages(FDUserI user) throws FDResourceException {
		...
		String subTotal = JspMethods.formatPrice(user.getShoppingCart().getSubTotal());
		String minimumOrder = JspMethods.formatPrice(user.getMinimumOrderAmount());

		warningMessages.put(ALCOHOL_PICKUP_BELOW_MINIMAL_ORDER, "Because your order falls below our " + minimumOrder
				+ " minimum when Wines & Spirits is removed, please return to your cart to add items before continuing checkout.");
		warningMessages.put(ALCOHOL_BELOW_MINIMAL_ORDER, "Because your order falls below our " + minimumOrder
				+ " minimum when alcohol is removed, please return to your cart to add items before continuing checkout.");
		warningMessages.put(PICKUP_BELOW_MINIMAL_ORDER, "We're sorry, you cannot check out because there is a " + minimumOrder
				+ " pre-tax cart minimum for all FreshDirect pickup orders. To check out with a pickup location, please add more items to your cart first.");
		warningMessages.put(BELOW_MINIMAL_ORDER, "We're sorry, you cannot check out because your pretax order total of " + subTotal + " is under the FreshDirect " + minimumOrder
	...
	public String selectWarningType(FDUserI user) throws FDResourceException {
		...
		if (cart.containsDlvPassOnly()) {
			...
			if (EnumServiceType.PICKUP.equals(deliveryAddress.getServiceType())) {
				warningType = PICKUP_BELOW_MINIMAL_ORDER;
			} else {
				warningType = BELOW_MINIMAL_ORDER;
			}
		}
	...
	public void adjustCartAvailability(final HttpServletRequest request, List<String> removeCartLineIds, final FDUserI user) throws FDResourceException, FDInvalidConfigurationException {
		FDCartModel cart = user.getShoppingCart();
