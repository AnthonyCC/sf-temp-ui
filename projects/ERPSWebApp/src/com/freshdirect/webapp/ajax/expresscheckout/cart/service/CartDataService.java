...
public class CartDataService {
	...
	public FormDataResponse validateOrderMinimumOnStartCheckout(FDUserI user, FormDataRequest request) throws FDResourceException {
		...
		if (orderMinimumWarningMessageKey != null) {
			...
			headerMessageMap.put("warningMessage", populateWarningMessage(orderMinimumWarningMessageKey, user));
			result.getSubmitForm().getResult().put("viewCartHeaderMessage", headerMessageMap);
		} else {
	...
	public CartData.Item populateCartDataItem(FDCartLineI cartLine, FDProduct fdProduct, ItemCount itemCount, FDCartI cart, Set<Integer> recentIds, ProductModel productNode, FDUserI user)
	...
	private void populateCartOrderData(FDUserI user, HttpServletRequest request, String userId, FDCartI cart, CartData cartData, Set<Integer> recentIds) throws HttpErrorResponse {
		try {
			...
			cartData.setWarningMessage(populateWarningMessage(request.getParameter("warning_message"), user));
	...
	private String populateWarningMessage(String warningMessageKey, FDUserI user) throws FDResourceException {
		...
		if (warningMessageKey != null) {
			...
			if (populateWarningMessages.containsKey(warningMessageKey)) {
	...
	private void updateCartLinesByChangeType(FDUserI user, FDCartModel cart, List<FDCartLineI> clines2report, String serverName, FDCartLineI cartLine, CartRequestData.Change change, String chType) {
