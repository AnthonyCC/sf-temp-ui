			...
			if (promotion == null) {// This check is required for incomplete
				...
				if (!eligible) {
					if (user.isFraudulent() && promotion.isFraudCheckRequired()) {
						result.add(new ValidationError(PROMOTION_CODE_FIELD_ID, MessageFormat.format(SystemMessageList.MSG_PROMO_NOT_UNIQUE_INFO, new Object[] { user.getCustomerServiceContact() })));
						...
						if (errorCode == PromotionErrorType.NO_ELIGIBLE_ADDRESS_SELECTED.getErrorCode() || errorCode == PromotionErrorType.NO_DELIVERY_ADDRESS_SELECTED.getErrorCode()) {
						...
						} else if (errorCode == PromotionErrorType.NO_ELIGIBLE_PAYMENT_SELECTED.getErrorCode() || errorCode == PromotionErrorType.NO_PAYMENT_METHOD_SELECTED.getErrorCode()) {
					...
					/*
					 * The redemption promotion is applied. Check if there is
					 * any previously applied header discount-the automatice
					 * ones. If yes then throw an error message to the user.
					 */
