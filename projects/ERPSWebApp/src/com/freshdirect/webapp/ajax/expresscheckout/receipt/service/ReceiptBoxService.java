...
public class ReceiptBoxService {
	...
	private static final String APPLY_MY_GIFT_CARD_BALANCE_NAME = "Apply my Gift Card Balance";
	private static final String APPLY_MY_GIFT_CARD_BALANCE_ID = "applymygiftcardbalance";
	...
	private static final String FREE_FOOD_TEXT = "";
	...
	public void populateApplyMyGiftBalanceToBox(List<CartSubTotalFieldData> receiptBox, FDUserI user, String requestURI) {
		if (user.getGiftcardBalance() > 0) {
			CartSubTotalFieldData data = new CartSubTotalFieldData();
			data.setId(APPLY_MY_GIFT_CARD_BALANCE_ID);
			data.setText(APPLY_MY_GIFT_CARD_BALANCE_NAME);
	...
	public void populateDiscountsToBox(List<CartSubTotalFieldData> receiptBox, FDOrderI order) {
		if (order.getTotalDiscountValue() >= 0) {
			...
			for (ErpDiscountLineModel discountLineModel : discountLineModels) {
				...
				freeFoodLine.setId(FREE_FOOD_ID);
				freeFoodLine.setText(FREE_FOOD_TEXT);
				freeFoodLine.setValue(desc);
				receiptBox.add(freeFoodLine);
			}
		}
	}
