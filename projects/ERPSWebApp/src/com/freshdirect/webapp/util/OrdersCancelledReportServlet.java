...
public class OrdersCancelledReportServlet extends AbstractExcelReportServlet {
	
	private static Category LOGGER = LoggerFactory.getInstance(OrdersForResvReportServlet.class);
	
	protected String getWorksheetName() {
		return "Orders Cancellation Report";
	}
	...
	private List getStandardReport(HttpServletRequest request) throws FDResourceException {
		...
		GenericSearchCriteria criteria = (GenericSearchCriteria)session.getAttribute("CANCEL_ORDERS_CRITERIA");
		if(criteria == null ){
			LOGGER.error("Session Data CANCEL_ORDERS_CRITERIA is missing.");
		...
		if(orders == null || orders.size() == 0){
			LOGGER.error("The Cancellation order search return null or empty List.");
