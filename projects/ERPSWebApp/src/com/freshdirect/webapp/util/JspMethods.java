...
public class JspMethods {
	...
	public static String getDisplayName( ContentNodeModel content_node, ProductModel prodNode ) {
		if ( content_node == null || prodNode == null
				|| !( ContentNodeI.TYPE_CATEGORY.equals( content_node.getContentType() ) ) )
			return "";
	...
	public static List sorter( java.util.Collection CatsAndProds, String orderBy, boolean reverseOrder,
	...
	// *******************************************************
	public static String displayFAProducts( LinkedList productLinks, LinkedList productPrices, boolean showPrices,
	...
	public static DisplayObject loadLayoutDisplayStrings( HttpServletResponse response, String categoryId,
			ContentNodeModel displayThing, String productNameAttribute ) throws JspException {
		return loadLayoutDisplayStrings( response, categoryId, displayThing, productNameAttribute, true, false, null,
				false );
	}

	public static DisplayObject loadLayoutDisplayStrings( HttpServletResponse response, String categoryId,
			ContentNodeModel displayThing, String productNameAttribute, boolean showPrice ) throws JspException {
		return loadLayoutDisplayStrings( response, categoryId, displayThing, productNameAttribute, showPrice, false,
				null, false );
	}

	public static DisplayObject loadLayoutDisplayStrings( HttpServletResponse response, String categoryId,
			ContentNodeModel displayThing, String productNameAttribute, boolean showPrice, boolean gotoPrimaryHome,
			String trkCode ) throws JspException {
		return loadLayoutDisplayStrings( response, categoryId, displayThing, productNameAttribute, showPrice,
				gotoPrimaryHome, trkCode, false );
	}

	public static DisplayObject loadLayoutDisplayStrings( HttpServletResponse response, String categoryId,
			ContentNodeModel displayThing, String productNameAttribute, boolean showPrice, boolean gotoPrimaryHome,
			String trkCode, boolean useAltImage ) throws JspException {
		// String itemImage = "";
		DisplayObject displayObj = new DisplayObject();
		String trackingCode = ( trkCode == null || "".equals( trkCode.trim() ) ) ? "" : "&trk=" + trkCode;
		...
		if ( displayThing.getContentType().equals( ContentNodeI.TYPE_PRODUCT ) ) {
			...
			try {
				if ( dfltSku != null && showPrice ) {
					...
					if ( !"nm".equalsIgnoreCase( salesUnitDescr ) && !"ea".equalsIgnoreCase( salesUnitDescr )
			...
			if ( gotoPrimaryHome ) {
				...
				if ( attrib != null ) {
					...
					itemURL = response.encodeURL( "product.jsp?productId=" + displayProduct + "&catId="
							+ catRef.getRefName() + trackingCode );
				}
			} else {
				itemURL = response.encodeURL( "product.jsp?productId=" + displayProduct + "&catId=" + categoryId
						+ trackingCode );
			}
			 ...
			 * indicators.append("<IMG SRC=\""); indicators.append(organicIndicator);
			 * indicators.append("\" width=\"5\" height=\"5\" hspace=\"0\" vspace=\"5\" border=\"0\"");
			 * indicators.append(" ALT=\"\">"); } else{
			 */
			indicators.append( "&nbsp;" );
			// }
			...
			if ( false == true ) {// !!! need another way to do this(displayFolder.getBoolean("ATR_contains_organic",
									// false)) {
				indicators.append( "<IMG SRC=\"" );
	...
	public static String removeChars( String source, String charsToRemove ) {
		if ( source == null || charsToRemove == null || source.length() == 0 || charsToRemove.length() == 0 )
			return source;
