...
public class PageViewTagModelBuilder {
    ...
    public PageViewTagModel buildTagModel() throws SkipTagException {
        if (input == null) {
            throw new SkipTagException("Failed to build CM PageViewTag", new NullPointerException("No input specified, abort now!"));
        }
    ...
    private void identifyPageAndCategoryId() throws SkipTagException {
        if (input.uri == null) {
            // CRASH NOW!
            throw new SkipTagException("Failed to build CM PageViewTag", new NullPointerException("Missing / NULL 'uri' property"));
        }
        ...
        if (slashAfterDirNamePos > -1) {
            ...
            } else if ("department".equals(fileName) || "department_cohort_match".equals(fileName) || "category".equals(fileName) || "newsletter".equals(fileName)
                    || "whatsgood".equals(fileName) || "ddpp".equals(fileName) /* || "browse".equals(fileName) */) {
                processDeptOrCat();
        ...
        if (tagModel.getCategoryId() == null) {
            ...
            } else if (uriAfterSlash.contains("cart.jsp") || uriAfterSlash.contains("confirm.jsp") || uriAfterSlash.contains("cart_confirm_pdp.jsp")
                    || uriAfterSlash.contains("quickbuy") || uriAfterSlash.contains("shop5.jsp") || uriAfterSlash.contains("product_modify.jsp")) {
                tagModel.setCategoryId(CustomCategory.CART.toString());

            } else if (uriAfterSlash.contains("cheese/101_") || uriAfterSlash.contains("coffee/coffee_") || uriAfterSlash.contains("producers_map.jsp")
                    || uriAfterSlash.contains("peakproduce.jsp") || uriAfterSlash.contains("rating_ranking.jsp") || uriAfterSlash.contains("seasonal_guide.jsp")
    ...
    /**
     * Page view tags only fire for search pages if the searchResultsSize and searchTerm is set. Therefore the page view tag placed at the top of all pages (which doesn't have
     * these pieces of information yet) won't fire. But the additional page view tags placed in the search and faq_search pages will.
     * 
