...
public class ElementTagModelBuilder {
    ...
    public ElementTagModel buildTagModel() throws SkipTagException {
        ...
        if (user != null && model.getAttributesMaps().get(6) == null) {
            model.getAttributesMaps().put(6, user.getCohortName());
        }
    ...
    private void processTimeslot() throws SkipTagException {
        ...
        if (timeSlot != null) {
            String elementId = DateUtil.formatDayOfWeek(timeSlot.getDeliveryDate()) + "_" + DateUtil.formatCmTimeslot(timeSlot.getStartTime()) + "_"
                    + DateUtil.formatCmTimeslot(timeSlot.getEndTime());
            model.setElementId(elementId);
            model.setElementCategory(elementCategory);
            model.getAttributesMaps().put(1,
                    DateUtil.formatDayOfWeek(timeSlot.getDeliveryDate()) + "|" + DateUtil.getDiffInMinutes(timeSlot.getEndTime(), timeSlot.getStartTime()) / 60 + " h window");
    ...
    private void processSearchSort() throws SkipTagException {
        ...
        if (searchNavigator != null && !searchNavigator.isSortByDefault()) {
            // as the compareTo method of Score class is working in a reverse way (when the navigator says asc it will sort desc in order to keep the most popular things on the top
