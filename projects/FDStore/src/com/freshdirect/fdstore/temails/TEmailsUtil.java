...
public final class TEmailsUtil {
	...
	public static TEmailContextI getTranEmailContext(EnumTranEmailType tranType, EnumTEmailProviderType provider,
			Map input) {
		...
		case EnumTEmailProviderType.CHEETAH_IDX:
			// CheetahTEmailContextImpl tmpContext=new
			// CheetahTEmailContextImpl();
			context = new CheetahTEmailContextImpl();
			if (EnumTranEmailType.ORDER_SUBMIT == tranType) {
				context.setCustomer((FDCustomerInfo) input.get(TEmailConstants.CUSTOMER_INP_KEY));
				context.setOrder((FDOrderI) input.get(TEmailConstants.ORDER_INP_KEY));
		...
		case EnumTEmailProviderType.SILVERPOP_IDX:
			if (EnumTranEmailType.CUST_SIGNUP == tranType) {
				context = new SilverpopTEmailContextImpl();
				context.setCustomer((FDCustomerInfo) input.get(TEmailConstants.CUSTOMER_INP_KEY));
				// tmpContext.setOrder((FDOrderI)input.get(TEmailConstants.ORDER_INP_KEY));
			}
			break;
		...
		//
		//
		// throw new FDRuntimeException("We are not supporting the provider
		// :"+provider.getName()+"at this time");
	}
	...
	public static TransEmailInfoModel createTransEmailModel(TEmailTemplateInfo info, Map input, String content) {
		...
		if (recipent == null)
			model.setRecipient(custInfo.getEmailAddress());
		else
			model.setRecipient(recipent);
		List list = (List) input.get(TEmailConstants.BCC_INP_KEY);
		if (list != null)
			model.setBCCList(list);

		List ccList = (List) input.get(TEmailConstants.CC_INP_KEY);
		if (ccList != null)
			model.setCCList(ccList);
		...
		if (input.get(TEmailConstants.OAS_QUERY) != null)
