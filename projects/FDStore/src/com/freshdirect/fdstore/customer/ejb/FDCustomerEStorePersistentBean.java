...
public class FDCustomerEStorePersistentBean extends DependentPersistentBeanSupport{
	...
	public PrimaryKey create(Connection conn) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("INSERT INTO CUST.FDCUSTOMER_ESTORE (FDCUSTOMER_ID, E_STORE, DEFAULT_SHIPTO, DEFAULT_PAYMENT, DEFAULT_DEPOT_LOC,EMAIL_OPTIN,TC_AGREE_DATE,TC_AGREE,DP_FREE_TRIAL_OPTIN) values (?,?,?,?,?,?,?,?,?)");
	...
	public void load(Connection conn) throws SQLException {
		...
		try {
			ps = conn.prepareStatement(
					"SELECT DEFAULT_SHIPTO, DEFAULT_PAYMENT, DEFAULT_DEPOT_LOC,TC_AGREE,EMAIL_OPTIN,DP_FREE_TRIAL_OPTIN FROM CUST.FDCUSTOMER_ESTORE WHERE FDCUSTOMER_ID=? AND E_STORE=?");
			ps.setString(1, this.getParentPK().getId());
			ps.setString(2, eStoreId.getContentId());
			rs = ps.executeQuery();
			if (rs.next()) {
				...
				model.setDpFreeTrailOptin("Y".equalsIgnoreCase(rs.getString("DP_FREE_TRIAL_OPTIN")) ? true : false);
	...
	public void store(Connection conn) throws SQLException {
		...
		try {
			...
			ps.setString(4, model.getDpFreeTrailOptin() ? "Y":"N");
