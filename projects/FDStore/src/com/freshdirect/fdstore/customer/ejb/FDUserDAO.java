...
public class FDUserDAO {
    ...
    private static FDUser createUser(Connection conn, String cookie, String street, String apt, String zipCode, String depotCode, EnumServiceType serviceType, EnumEStoreId eStoreId)
            throws SQLException {
     ...
     * @throws SQLException
     */
    private static void createFDUserEStore(Connection conn, String zipCode, EnumServiceType serviceType, EnumEStoreId eStoreId, String id) throws SQLException {
        PreparedStatement ps = null;
        try {
    ...
    private static void loadCart(Connection conn, FDUser user, final boolean lazy) throws SQLException {
        ...
        CatalogKey catalogKey = new CatalogKey(user.getUserContext().getStoreContext().getEStoreId().name(), Long.parseLong(user.getUserContext().getFulfillmentContext()
    ...
    private static FDUser loadUserFromResultSet(ResultSet rs, boolean populateUserContext) throws SQLException {
        ...
        if (rs.next()) {
            ...
            user.setZipCheckPopupUsed(NVL.apply(rs.getString("ZIP_POPUP_USED"), "").equalsIgnoreCase("X") ? true : false);
        } else {
            user = new FDUser();
        }
    ...
    private static final String STORE_USER_SQL = "UPDATE CUST.FDUSER "
            + "SET COOKIE=?, ZIPCODE=?, FDCUSTOMER_ID=?, DEPOT_CODE=?, SERVICE_TYPE=?, ADDRESS1=?, APARTMENT=?, "
            + "LAST_REF_PROG_ID=?, REF_PROG_INVT_ID=?, REF_TRK_KEY_DTLS=?, HPLETTER_VISITED=?, GLOBALNAVTUT_VIEWED=?, ZIP_POPUP_USED=?, CAMPAIGN_VIEWED=?, COHORT_ID=?, ZP_SERVICE_TYPE=?, DEFAULT_LIST_ID=? "
            + "WHERE ID=?";
    ...
    public static void storeMobilePreferences(Connection conn, String fdCustomerId, String mobileNumber, String textOffers, String textDelivery, String orderNotices,
            String orderExceptions, String offers, String partnerMessages, EnumEStoreId eStoreId) {
        PreparedStatement ps = null;
        ...
        try {
            ...
            if (eStoreId.getContentId().contentEquals(EnumEStoreId.FD.getContentId())) {
                ...
                if (existingMobNum != null && phone.getPhone() != null && !phone.getPhone().isEmpty() && phone.getPhone().length() != 0 && !phone.getPhone().equals(existingMobNum)) {
                    ps = conn
                            .prepareStatement("UPDATE CUST.FDCUSTOMER_ESTORE set mobile_number=?, ORDER_NOTIFICATION=?, ORDEREXCEPTION_NOTIFICATION=?, SMS_OFFERS_ALERT=?, PARTNERMESSAGE_NOTIFICATION=?, "
                    ...
                    if ((exceptions.equals(EnumSMSAlertStatus.SUBSCRIBED.value()) || exceptions.equals(EnumSMSAlertStatus.PENDING.value()))
                            && "y".equalsIgnoreCase(orderExceptions)) {
    ...
    public static void storeAllMobilePreferences(Connection conn, String customerId, String fdCustomerId, String mobileNumber, String textOffers, String textDelivery,
            String goGreen, String phone, String ext, boolean isCorpUser, EnumEStoreId eStoreid) {
        ...
        try {
            if (isCorpUser) {
                ps = conn
                        .prepareStatement("update CUST.CUSTOMERINFO set go_green=?,business_phone=replace(replace(replace(replace(replace(?,'('),')'),' '),'-'),'.'),business_ext=?,mobile_preference_flag=?  where customer_id=?");
                ps1 = conn
                        .prepareStatement("update CUST.FDCUSTOMER_ESTORE set mobile_number=?, offers_notification=?,delivery_notification=?  where FDCUSTOMER_ID=? and e_store=?");
            } else {
                ps = conn
                        .prepareStatement("update CUST.CUSTOMERINFO set go_green=?, home_phone = replace(replace(replace(replace(replace(?,'('),')'),' '),'-'),'.'), home_ext = ?,mobile_preference_flag=? where customer_id=?");
                ps1 = conn
                        .prepareStatement("update CUST.FDCUSTOMER_ESTORE set mobile_number=?, offers_notification=?,delivery_notification=?  where FDCUSTOMER_ID=? and e_store=?");
            }
    ...
    public static void logIpLocatorEvent(Connection conn, IpLocatorEventDTO ipLocatorEventDTO) {
        ...
        try {
            ps = conn
                    .prepareStatement("INSERT INTO MIS.IPLOCATOR_EVENT_LOG "
    ...
    public static IpLocatorEventDTO loadIpLocatorEvent(Connection conn, String fdUserId) {
        ...
        try {
            ps = conn
                    .prepareStatement("SELECT ID, INSERT_TIMESTAMP, IP, IPLOC_ZIPCODE, IPLOC_COUNTRY, IPLOC_REGION, IPLOC_CITY, FDUSER_ID, FD_ZIPCODE, FD_STATE, FD_CITY, USER_AGENT, UA_HASH_PERCENT, IPLOC_ROLLOUT_PERCENT "
    ...
    public static CustomerAvgOrderSize getHistoricOrderSize(Connection conn, String customerId) throws SQLException {
        ...
        PreparedStatement ps = conn.prepareStatement(ORDERSIZE_ESTIMATION_QUERY);
        ps.setString(1, customerId);
        ps.setInt(2, 5);
    ...
    public static boolean isReadyForPick(Connection conn, String id) throws SQLException {
        ...
        PreparedStatement ps = conn.prepareStatement(IS_READY_FOR_PICK);
        ps.setString(1, id);
    ...
    public static int getAddonOrdersCount(String orderId, Connection conn) throws SQLException {
        ...
        pstmt = conn
                .prepareStatement("select count(1) from CUST.SALE S, CUST.SALESACTION SA, CUST.PAYMENTINFO PI where S.ID = SA.SALE_ID AND S.CROMOD_DATE = SA.ACTION_DATE AND"
	...
	public static int resetDefaultPaymentValueType(Connection conn, String custId) {
        try {
			return conn.createStatement().executeUpdate("update cust.fdcustomer set DEFAULT_PAYMENT='', DEFAULT_PAYMENT_METHOD_TYPE='UD' where id="+custId);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return 0;		
