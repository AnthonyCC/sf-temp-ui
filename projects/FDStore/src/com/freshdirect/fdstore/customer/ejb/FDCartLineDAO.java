...
public class FDCartLineDAO {
	...
	private final static String QUERY_CARTLINES =
            "SELECT ID, SKU_CODE, VERSION, QUANTITY, SALES_UNIT, CONFIGURATION, RECIPE_SOURCE_ID, REQUEST_NOTIFICATION, VARIANT_ID, ADDED_FROM_SEARCH, DISCOUNT_APPLIED, SAVINGS_ID, ADDED_FROM, EXTERNAL_AGENCY, EXTERNAL_SOURCE, EXTERNAL_GROUP, E_STORE, SOURCE"
	...
	public static List<ErpOrderLineModel> loadCartLines(Connection conn, PrimaryKey fdUserPk, EnumEStoreId eStoreId) throws SQLException {
		...
		try {
			ps = conn.prepareStatement(QUERY_CARTLINE_CLIENTCODES);
			...
			while (rs.next()) {
				...
				line.setSku(sku);
				FDConfiguration config = new FDConfiguration(rs.getDouble("QUANTITY"), rs.getString("SALES_UNIT"),
				...
				if (rs.getString("DISCOUNT_APPLIED") != null
						&& rs.getString("DISCOUNT_APPLIED").equalsIgnoreCase("X")) {
	...
	public static void storeCartLines(Connection conn, PrimaryKey fdUserPk, List<ErpOrderLineModel> erpOrderlines,StoreContext storeContext) throws SQLException {
		...
		try {
			ps = conn.prepareStatement("DELETE FROM CUST.FDCARTLINE_CLIENTCODE WHERE FDUSER_ID = ?");
			ps.setString(1, fdUserPk.getId());
			ps.executeUpdate();
			ps.close();
			ps = conn.prepareStatement(
					"DELETE FROM CUST.FDCARTLINE WHERE FDUSER_ID = ? AND NVL(E_STORE,'FreshDirect')=? AND MOD_ORDER_ID IS NULL");
			ps.setString(1, fdUserPk.getId());
			...
			if (erpOrderlines.size() == 0)
				return;
			ps = conn.prepareStatement(
                    "INSERT INTO CUST.FDCARTLINE (ID, FDUSER_ID, SKU_CODE, VERSION, QUANTITY, SALES_UNIT, CONFIGURATION, RECIPE_SOURCE_ID, REQUEST_NOTIFICATION, VARIANT_ID, DISCOUNT_APPLIED, SAVINGS_ID, ADDED_FROM_SEARCH, ADDED_FROM, EXTERNAL_AGENCY, EXTERNAL_SOURCE, EXTERNAL_GROUP, E_STORE, SOURCE) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			for (ErpOrderLineModel line : erpOrderlines) {
                ...
                ps.setString(14, null != line.getAddedFrom() ? line.getAddedFrom().getName() : null);
                ps.setString(15, StringUtil.crop(line.getExternalAgency(), 30));
                ps.setString(16, StringUtil.crop(line.getExternalSource(), 30));
                ps.setString(17, StringUtil.crop(line.getExternalGroup(), 256));
                ps.setString(18,
						null != line.getEStoreId() ? line.getEStoreId().getContentId()
								: (null != storeContext && null != storeContext.getEStoreId()
										? storeContext.getEStoreId().getContentId() : EnumEStoreId.FD.getContentId()));
                ps.setString(19, (line.getSource() != null ? line.getSource().toString() : null));
	...
	public static void clearModifyCartlines(Connection conn, String mod_order_id) throws SQLException {
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("DELETE FROM CUST.FDCARTLINE WHERE MOD_ORDER_ID=?");
	...
	public static List<FDCartLineI> getModifiedCartlines(Connection conn, String orderId, UserContext userContext) throws SQLException {
		...
		try {
			ps = conn.prepareStatement(MODIFY_QUERY_CARTLINES);
	...
	public static void saveModifiedCartlines(Connection conn, PrimaryKey  userpk, StoreContext storeContext, FDCartLineI line, String orderId) throws SQLException {
		PreparedStatement ps = null;
		
		try {
			ps = conn.prepareStatement(
                    "INSERT INTO CUST.FDCARTLINE (ID, FDUSER_ID, SKU_CODE, VERSION, QUANTITY, SALES_UNIT, CONFIGURATION, RECIPE_SOURCE_ID, REQUEST_NOTIFICATION, VARIANT_ID, DISCOUNT_APPLIED, SAVINGS_ID, ADDED_FROM_SEARCH, ADDED_FROM, EXTERNAL_AGENCY, EXTERNAL_SOURCE, EXTERNAL_GROUP, E_STORE, SOURCE, MOD_ORDER_ID) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			ps.setString(1, line.getCartlineId());
			ps.setString(2, userpk.getId());
			ps.setString(3, line.getSku().getSkuCode());
			ps.setInt(4, line.getSku().getVersion());
			ps.setBigDecimal(5, new BigDecimal(String.valueOf(line.getQuantity())));
			ps.setString(6, line.getSalesUnit());
            ...
            ps.setString(18,
					null != line.getEStoreId() ? line.getEStoreId().getContentId()
							: (null != storeContext && null != storeContext.getEStoreId()
									? storeContext.getEStoreId().getContentId() : EnumEStoreId.FD.getContentId()));
            ps.setString(19, null /* (line.getSource()!=null? line.getSource().toString():null) */);
            ps.setString(20, orderId);
			ps.executeUpdate();
		} finally {
			DaoUtil.close(ps);
		}
	...
	public static void removeModifiedCartline(Connection conn, FDCartLineI cartLine) throws SQLException {
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("DELETE FROM CUST.FDCARTLINE WHERE ID =? and MOD_ORDER_ID IS NOT NULL");
			ps.setString(1, cartLine.getCartlineId());
			ps.executeUpdate();
		} finally {
			DaoUtil.close(ps);
		}	
	...
	public static void updateModifiedCartlineQuantity(Connection conn, FDCartLineI cartLine) throws SQLException {
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("UPDATE CUST.FDCARTLINE SET QUANTITY=? WHERE ID =? and MOD_ORDER_ID=?");
