...
public class CustomerIdentityService extends AbstractEcommService implements CustomerIdentityServiceI {
	...
	public FDIdentity login(String userId, String password) throws FDAuthenticationException, FDResourceException, RemoteException{
		...
		try {
			...
			response = this.postDataTypeMap(inputJson, getFdCommerceEndPoint(LOGIN), new TypeReference<Response<FDIdentity>>() {});
			
			if (!response.getResponseCode().equals("OK")) {
				if ("FDAuthenticationException".equals(response.getMessage())) {
					throw new FDAuthenticationException(response.getError().get("FDAuthenticationException").toString());
