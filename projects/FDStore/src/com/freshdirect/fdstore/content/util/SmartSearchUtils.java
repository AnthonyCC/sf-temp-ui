...
public class SmartSearchUtils {
    ...
    public static void collectSaleInfo(List<FilteringSortingItem<ProductModel>> products) {
		for (FilteringSortingItem<ProductModel> product : products) {
            ProductModel p = ProductPricingFactory.getInstance().getPricingAdapter(product.getModel());
			product.putSortingValue(EnumSortingValue.DEAL, p.getPriceCalculator().getHighestDealPercentage());
		}
	}

    public static void collectAvailabilityInfo(List<FilteringSortingItem<ProductModel>> products) {
		for (FilteringSortingItem<ProductModel> product : products) {
            ProductModel p = ProductPricingFactory.getInstance().getPricingAdapter(product.getModel());
    ...
    public static List<ProductModel> sortBySale(List<ProductModel> products, boolean ascending) {
