...
//    	user.setDefaultPricingContext();
    ...
    protected void populateHomePages(SessionUser user, CMSPageRequest pageRequest, PageMessageResponse pageResponse, HttpServletRequest request) throws FDException {
        ...
        if (isWebRequest) {
            final StoreModel ztore = ContentFactory.getInstance().getStore();
            
            List<ImageBanner> bannerz = ztore.getWelcomeCarouselImageBanners();
            if (bannerz == null || bannerz.isEmpty()) {
                pageResponse.setWelcomeCarouselBanners(Collections.<com.freshdirect.mobileapi.controller.data.ImageBanner>emptyList());
            } else {
                
                // transform image banners

                List<com.freshdirect.mobileapi.controller.data.ImageBanner> result = new ArrayList<com.freshdirect.mobileapi.controller.data.ImageBanner>( bannerz.size() );
                for (final ImageBanner banner : bannerz) {
                    
                    com.freshdirect.mobileapi.controller.data.ImageBanner imgBanner = com.freshdirect.mobileapi.controller.data.ImageBanner.wrap(banner);

                    result.add(imgBanner);
                }

                // put them into response
                
                pageResponse.setWelcomeCarouselBanners(result);
            }
        }
    }
