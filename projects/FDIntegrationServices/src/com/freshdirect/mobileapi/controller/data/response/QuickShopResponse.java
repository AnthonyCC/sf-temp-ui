...
public class QuickShopResponse extends Message {
    ...
    private List<ProductPotatoData> items;
    ...
    public QuickShopResponse(List<QuickShopLineItem> items, Map<FilteringValue, Map<String, FilteringMenuItem>> menu, QuickShopPagerValues pager, List<QuickShopSorterValues> sorter, String searchTerm, QuickShopListDetails listDetails) {
     ...
     */
    private void prepareItems(List<QuickShopLineItem> lineItems) {
        items = new ArrayList<ProductPotatoData>();
        for (QuickShopLineItem lineItem : lineItems) {
            ProductPotatoData productPotatoData = new ProductPotatoData();
            productPotatoData.setProductData(lineItem);
            items.add(productPotatoData);
        }
    }
    ...
    public QuickShopResponse(List<QuickShopLineItem> items, QuickShopPagerValues pager, List<QuickShopSorterValues> sorter) {
        prepareItems(items);
        prepareSorter(sorter);
        this.pager = pager;
    }

    public List<ProductPotatoData> getItems() {
        return items;
    }
