...
public class ProductPotatoUtil {
     ...
     */
    public static ProductPotatoData getProductPotato(final ProductModel product, final ServletContext servletContext, final FDUserI user, final boolean requiresExtraFields) {
        if (product != null) {
            ...
            try {
                ...
                if (requiresExtraFields) {
                    data.setProductExtraData( ProductExtraDataPopulator.createExtraData(user, product, servletContext, null, null));
                }
                return data;
     ...
     * @return
     */
    public static ProductPotatoData getProductPotato(final String productId, final String categoryId, final ServletContext servletContext, final FDUserI user, final boolean requiresExtraFields) {
        final ProductModel product = PopulatorUtil.getProduct( productId, categoryId );
        if (product != null) {
            return getProductPotato(product, servletContext, user, requiresExtraFields);
        } else {
    ...
    private static void populateProductConfiguration(final FDUserI user, final ProductConfiguration productConfiguration, final ServletContext servletContext) {
        ...
        ProductPotatoData potato =  getProductPotato( productId, categoryId, servletContext, user, false );
    ...
    public static void populateCartDetailWithPotatoes(final FDUserI user, final CartDetail cartDetail, final ServletContext servletContext) {
        ...
        for (final ProductConfiguration prodConf : configurations) {
            populateProductConfiguration(user, prodConf, servletContext);
        }
    }
}
