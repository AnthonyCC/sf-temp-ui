...
p.first_dlv_time = (p.truck_dispatchtime  +
    CASE 
         WHEN (p.zone is NULL AND p.is_bullpen='Y') THEN ((select decode(stem_to_time, NULL, 0, stem_to_time) from transp.zone z where z.zone_code=p.zone)  * (1/24/60))
...
p.last_dlv_time = (p.truck_endtime  - 
    CASE 
         WHEN (p.zone is NULL AND p.is_bullpen='Y') THEN ((select decode(stem_from_time, NULL, decode(stem_to_time, NULL, 0, stem_to_time), stem_from_time)  from transp.zone z where z.zone_code=p.zone)  * (1/24/60))
