	...
	BEGIN
		IF request_date IS NULL THEN
			...
			DBMS_OUTPUT.PUT_LINE ('Credited ' || SQL%ROWCOUNT || ' records with one delivery');
			 ...
			 /*  Update delivery pass status to RTU when the original order got delivered.   */	
			UPDATE cust.delivery_pass SET status='RTU' WHERE status IN ('PEN','STF') AND purchase_order_id IN (SELECT /*+ USE_NL(s, sa) */s.id FROM cust.sale s, cust.salesaction sa WHERE sa.requested_date >= trunc(SYSDATE -10) AND sa.sale_id = s.id AND sa.action_type IN ('CRO', 'MOD') AND s.status = 'PPG' AND sa.action_date = (SELECT MAX(action_date) FROM cust.salesaction WHERE sale_id = s.id AND action_type IN ('CRO', 'MOD')));
		...
		ELSE
			...
			DBMS_OUTPUT.PUT_LINE ('Credited ' || SQL%ROWCOUNT || ' records with one delivery');
			 ...
			 /*  Update delivery pass status to RTU when the original order got delivered.   */	
			UPDATE cust.delivery_pass SET status='RTU' WHERE status IN ('PEN','STF') AND purchase_order_id IN (SELECT /*+ USE_NL(s, sa) */s.id FROM cust.sale s, cust.salesaction sa WHERE sa.requested_date = TRUNC(request_date) AND sa.sale_id = s.id AND sa.action_type IN ('CRO', 'MOD') AND s.status = 'PPG' AND sa.action_date = (SELECT MAX(action_date) FROM cust.salesaction WHERE sale_id = s.id AND action_type IN ('CRO', 'MOD')));
