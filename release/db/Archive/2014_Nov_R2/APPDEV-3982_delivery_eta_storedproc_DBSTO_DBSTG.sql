...
BEGIN
           ...
           SELECT bs.weborder_id,
                  ...
                  CASE
                     ...
                     THEN
                        GREATEST (
                           DECODE (y.ROUTE_MAXSCANTIME,
                                   NULL, T.ROUTE_MIN_WINDOWSTARTTIME,
                                   y.ROUTE_MAXSCANTIME)
                           + SUM (
             ...
             FROM TRANSP.HANDOFF_BATCH b,
                  ...
                  (  SELECT BSX.ROUTE_NO,
                            bsx.ROUTING_ROUTE_NO,
                            MIN (BSX.WINDOW_STARTTIME)
                               AS ROUTE_MIN_WINDOWSTARTTIME
                       FROM TRANSP.HANDOFF_BATCH bx, TRANSP.HANDOFF_BATCHSTOP bsx
                      WHERE     bx.batch_id = bsx.batch_id
                            AND bx.delivery_date = TRUNC (SYSDATE)
                            AND bx.batch_status IN ('CPD', 'CPD/ADC', 'CPD/ADF')
                   GROUP BY BSX.ROUTE_NO, BSX.ROUTING_ROUTE_NO) T
            WHERE     B.BATCH_ID = BS.BATCH_ID
                  ...
                  AND BS.ROUTING_ROUTE_NO = T.ROUTING_ROUTE_NO
                  AND BS.ROUTE_NO = T.ROUTE_NO
   ...
   BEGIN
      ...
      DBMS_OUTPUT.
      put_line ('Order delivery metric Stored Procedure completed Successfully');
      COMMIT;
   EXCEPTION
      WHEN OTHERS
      THEN
