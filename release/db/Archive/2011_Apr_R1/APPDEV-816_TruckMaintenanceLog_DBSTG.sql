--Please execute below scripts as TRANSP. schema owner

--
-- Definition for table "TRANSP.VIRRecord"
--

CREATE TABLE TRANSP.VIRRecord  
(
    id VARCHAR2(30),   
    TruckNumber VARCHAR2 (10) NOT NULL , 
    VendorName VARCHAR2 (40) ,
    ReportingDriver VARCHAR2 (60) DEFAULT  '' NOT NULL , 
    CreatedBy VARCHAR2 (10) NOT NULL , 
    DateCreated  DATE NOT NULL , 
    PRIMARY KEY (id)
);

CREATE  SEQUENCE  TRANSP.VIRRecord_id_SEQ START WITH 1 INCREMENT BY 1;

CREATE INDEX TRANSP.FK_VIRRecord_AuthUser ON TRANSP.VIRRecord  ( CreatedBy );

CREATE INDEX TRANSP.FK_VIRRecord_TruckNumber ON TRANSP.VIRRecord  ( TruckNumber );

--
-- Definition for table "TRANSP.IssueLog"
--
CREATE TABLE TRANSP.IssueLog  
(
    id VARCHAR2(30), 
    VIRRECORD_ID VARCHAR2(30),       
    IssueType VARCHAR2(45) NOT NULL , 
    IssueSubType VARCHAR2(45) NOT NULL , 
    Comments VARCHAR2 (200) DEFAULT  '' , 
    IssueLocation VARCHAR2 (40) DEFAULT  '' NOT NULL , 
    IssueSide VARCHAR2 (40) DEFAULT  '' NOT NULL,
    MaintenanceIssueID VARCHAR2(30) ,     
    PRIMARY KEY (id) 
);

CREATE SEQUENCE  TRANSP.IssueLog_id_SEQ START WITH 1 INCREMENT BY 1;

CREATE INDEX TRANSP.FK_IssueLog_IssueSubType ON TRANSP.IssueLog  ( IssueSubType );

CREATE INDEX TRANSP.FK_IssueLog_IssueType ON TRANSP.IssueLog  ( IssueType );

--
-- Definition for table "MaintenanceIssueID"
--
CREATE TABLE TRANSP.MAINTENANCEISSUE
(
  ID                    VARCHAR2(30 BYTE),
  TRUCKNUMBER           VARCHAR2(45 BYTE)       NOT NULL,
  VENDORNAME            VARCHAR2(45 BYTE)       NOT NULL,
  ISSUETYPE             VARCHAR2(45)              NOT NULL,
  ISSUESUBTYPE          VARCHAR2(45)             NOT NULL,
  ISSUESTATUS           VARCHAR2(20 BYTE)       DEFAULT 'Open',
  COMMENTS              VARCHAR2(200 BYTE)      DEFAULT '',
  ISSUELOCATION         VARCHAR2(40 BYTE)       DEFAULT ''                    NOT NULL,
  ISSUESIDE             VARCHAR2(20 BYTE)       DEFAULT ''                    NOT NULL,
  DATECREATED           DATE                    NOT NULL,
  MODIFIEDDATE          DATE                    NOT NULL,
  CREATEDBY             VARCHAR2(40 BYTE)       NOT NULL,
  SERVICESTATUS         VARCHAR2(20 BYTE)       DEFAULT 'In Service',
  ESTIMATED_REPAIRDATE  DATE,
  ACTUAL_REPAIRDATE     DATE,
  DATE_VERIFIED         DATE,
  VERIFIEDBY            VARCHAR2(20 BYTE)       DEFAULT '',
  REPAIREDBY            VARCHAR2(20 BYTE)       DEFAULT '',
  PRIMARY KEY (id)
)

CREATE  SEQUENCE  TRANSP.MaintenanceIssue_id_SEQ  START WITH 1  INCREMENT BY 1;

CREATE INDEX TRANSP.FK_MaintenanceLog_IssueSubType ON TRANSP.MAINTENANCEISSUE  ( ISSUESUBTYPE );

CREATE INDEX TRANSP.FK_MaintenanceLog_IssueType ON TRANSP.MAINTENANCEISSUE  ( ISSUETYPE );

CREATE INDEX TRANSP.FK_MaintenanceLog_VerifiedBy ON TRANSP.MAINTENANCEISSUE  ( VERIFIEDBY );

CREATE INDEX TRANSP.FK_MaintenanceLog_SrcStatus ON TRANSP.MAINTENANCEISSUE  ( SERVICESTATUS );

--
-- Definition for table "IssueSubType"
--

CREATE TABLE TRANSP.IssueSubType  
(
    id NUMBER (10), 
    IssueTypeId NUMBER (10) NOT NULL , 
    IssueSubTypeName VARCHAR2 (45) DEFAULT  '' NOT NULL , 
    IssueSubTypeDescription VARCHAR2 (45) DEFAULT  '' NOT NULL , 
    isActive NUMBER (10) DEFAULT  '0' NOT NULL , 
    CreatedBy VARCHAR2 (45) NOT NULL , 
    DateCreated DATE  NOT NULL , 
    PRIMARY KEY (id) , 
    UNIQUE (IssueSubTypeName, IssueTypeId)
);

CREATE  SEQUENCE TRANSP.IssueSubType_id_SEQ   START WITH 1 INCREMENT BY 1;

CREATE INDEX TRANSP.FK_IssueSubType_IssueTypeID ON TRANSP.IssueSubType  ( IssueTypeId );

CREATE INDEX TRANSP.FK_IssueSubType_UserAuthID ON TRANSP.IssueSubType  ( CreatedBy );

--
-- Definition for table "TRANSP.IssueType"
--

CREATE TABLE TRANSP.IssueType  
(
    id NUMBER (10), 
    IssueTypeName VARCHAR2 (45) DEFAULT  '' NOT NULL , 
    IssueTypeDescription VARCHAR2 (45) DEFAULT  '' NOT NULL , 
    isActive NUMBER (10) DEFAULT  '0' NOT NULL , 
    CreatedBy VARCHAR2 (20) NOT NULL , 
    DateCreated DATE  NOT NULL , 
    PRIMARY KEY (id) , 
    UNIQUE (IssueTypeName) 
);

CREATE  SEQUENCE TRANSP.IssueType_id_SEQ  START WITH 1 INCREMENT BY 1;

CREATE INDEX TRANSP.FK_EventType_UserAuth ON TRANSP.IssueType  ( CreatedBy );

--
-- Foreign keys for table "IssueLog"
--
ALTER TABLE TRANSP.IssueLog
add
(
    CONSTRAINT FK_IssueLog_VIRRecord FOREIGN KEY (VIRRecord_id) REFERENCES TRANSP.VIRRecord (id) ,  
    CONSTRAINT FK_IssueLog_MaintenanceIssue FOREIGN KEY (MaintenanceIssueID) REFERENCES TRANSP.MAINTENANCEISSUE (id)
);

--
-- Foreign keys for table "TRANSP.IssueSubType"
--
ALTER TABLE TRANSP.IssueSubType ADD CONSTRAINT FK_IssueSubType_IssueTypeID FOREIGN KEY (IssueTypeId) REFERENCES TRANSP.IssueType (id);

--
-- Definition for table "TRANSP.ASSET_ATRTEMPLATE"
--

CREATE TABLE TRANSP.ASSETTEMPLATE
(
  ATR_TEMPLATE_ID          VARCHAR2(40 BYTE),
  ATR_TEMPLATE_NAME     VARCHAR2(40 BYTE)          NOT NULL,
  ASSET_TYPE         VARCHAR2(40 BYTE)          NOT NULL ,
  PRIMARY KEY (ATR_TEMPLATE_ID) 
);

CREATE TABLE TRANSP.ASSETTEMPLATE_ATTRIBUTE
(
  ATR_TEMPLATE_ID           VARCHAR2(40 BYTE),
  ATTRIBUTE_TYPE   VARCHAR2(40 BYTE)            NOT NULL,
  ATTRIBUTE_VALUE  VARCHAR2(40 BYTE)            NOT NULL,
  PRIMARY KEY (ATR_TEMPLATE_ID, ATTRIBUTE_TYPE)  
);

--
-- Foreign keys for table "TRANSP.ASSET_ATRTEMPLATE"
--
ALTER TABLE TRANSP.ASSETTEMPLATE ADD(CONSTRAINT FK_ASSETTYPE_ID FOREIGN KEY (ASSET_TYPE) REFERENCES  TRANSP.ASSET_TYPE (CODE));

--
-- Foreign keys for table "ASSETTEMPLATE_ATTRIBUTE"
--
ALTER TABLE TRANSP.ASSETTEMPLATE_ATTRIBUTE ADD
(
    CONSTRAINT FK_ASSETTEMPLATE_ID FOREIGN KEY (ATR_TEMPLATE_ID) REFERENCES  TRANSP.ASSETTEMPLATE (ATR_TEMPLATE_ID) ,
    CONSTRAINT FK_ATTRIBUTETYPE_ID FOREIGN KEY (ATTRIBUTE_TYPE) REFERENCES  TRANSP.ASSET_ATTRIBUTETYPE (CODE)
);

ALTER TABLE TRANSP.ASSET_ATTRIBUTETYPE add(ASSET_TYPE  VARCHAR2(40 BYTE)  NOT NULL);

ALTER TABLE TRANSP.ASSET_ATTRIBUTETYPE add(CONSTRAINT FK_ASSETTYPE FOREIGN KEY (ASSET_TYPE) REFERENCES  TRANSP.ASSET_TYPE (CODE));

ALTER TABLE TRANSP.ASSET add (ASSET_TEMPLATE       VARCHAR2(40 BYTE));

ALTER TABLE TRANSP.ASSET_ATTRIBUTE add(ATTRIBUTE_MATCH     VARCHAR2(1 BYTE));

CREATE INDEX TRANSP.IDX01_ASSETTEMPLATE ON TRANSP.ASSETTEMPLATE (ASSET_TYPE);

CREATE INDEX TRANSP.IDX01_ASSETTEMPLATE_ATTRIBUTE ON TRANSP.ASSETTEMPLATE_ATTRIBUTE (ATR_TEMPLATE_ID);

CREATE SEQUENCE TRANSP.ATTRIBUTE_TEMPLATE_SEQ   START WITH 1  INCREMENT BY 1;

CREATE SEQUENCE TRANSP.ASSETTEMPLATE_ATTRIBUTE_SEQ   START WITH 1  INCREMENT BY 1;

GRANT ALL ON TRANSP.VIRRecord_id_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.VIRRecord_id_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.VIRRecord_id_SEQ TO APPDEV;

GRANT ALL ON TRANSP.IssueLog_id_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.IssueLog_id_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.IssueLog_id_SEQ TO APPDEV;

GRANT ALL ON TRANSP.VIRRECORD TO fdtrn_ststg01;
GRANT ALL ON TRANSP.VIRRECORD TO fdstore_ststg01;
GRANT SELECT ON TRANSP.VIRRECORD TO APPDEV;

GRANT ALL ON TRANSP.IssueLog TO fdtrn_ststg01;
GRANT ALL ON TRANSP.IssueLog TO fdstore_ststg01;
GRANT SELECT ON TRANSP.IssueLog TO APPDEV;

GRANT ALL ON TRANSP.IssueSubType_id_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.IssueSubType_id_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.IssueSubType_id_SEQ TO APPDEV;

GRANT ALL ON TRANSP.IssueSubType TO fdtrn_ststg01;
GRANT ALL ON TRANSP.IssueSubType TO fdstore_ststg01;
GRANT SELECT ON TRANSP.IssueSubType TO APPDEV;

GRANT ALL ON TRANSP.IssueType TO fdtrn_ststg01;
GRANT ALL ON TRANSP.IssueType TO fdstore_ststg01;
GRANT SELECT ON TRANSP.IssueType TO APPDEV;

GRANT ALL ON TRANSP.IssueType_id_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.IssueType_id_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.IssueType_id_SEQ TO APPDEV;

GRANT ALL ON TRANSP.MaintenanceIssue_id_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.MaintenanceIssue_id_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.MaintenanceIssue_id_SEQ TO APPDEV;

GRANT ALL ON TRANSP.MaintenanceIssue TO fdtrn_ststg01;
GRANT ALL ON TRANSP.MaintenanceIssue TO fdstore_ststg01;
GRANT SELECT ON TRANSP.MaintenanceIssue TO APPDEV;

GRANT ALL ON TRANSP.ATTRIBUTE_TEMPLATE_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.ATTRIBUTE_TEMPLATE_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.ATTRIBUTE_TEMPLATE_SEQ TO appdev;

GRANT ALL ON TRANSP.ASSETTEMPLATE TO fdtrn_ststg01;
GRANT ALL ON TRANSP.ASSETTEMPLATE TO fdstore_ststg01;
GRANT SELECT ON TRANSP.ASSETTEMPLATE TO appdev;

GRANT ALL ON TRANSP.ASSETTEMPLATE_ATTRIBUTE_SEQ TO fdtrn_ststg01;
GRANT ALL ON TRANSP.ASSETTEMPLATE_ATTRIBUTE_SEQ TO fdstore_ststg01;
GRANT SELECT ON TRANSP.ASSETTEMPLATE_ATTRIBUTE_SEQ TO appdev;

GRANT ALL ON TRANSP.ASSETTEMPLATE_ATTRIBUTE TO fdtrn_ststg01;
GRANT ALL ON TRANSP.ASSETTEMPLATE_ATTRIBUTE TO fdstore_ststg01;
GRANT SELECT ON TRANSP.ASSETTEMPLATE_ATTRIBUTE TO appdev;




