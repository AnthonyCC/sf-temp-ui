CREATE OR REPLACE PROCEDURE CUST.DELIVERY_PASS_CRON(request_date IN DATE)
	...
	BEGIN
		IF request_date IS NULL THEN
			...
			UPDATE cust.delivery_pass SET status='ACT' WHERE status IN ('PEN','STF') AND purchase_order_id IN (SELECT /*+ USE_NL(s, sa) */s.id FROM cust.sale s, cust.salesaction sa WHERE sa.requested_date >= trunc(SYSDATE -10) AND sa.sale_id = s.id AND sa.action_type IN ('CRO', 'MOD') AND s.status = 'PPG' AND sa.action_date = (SELECT MAX(action_date) FROM cust.salesaction WHERE sale_id = s.id AND action_type IN ('CRO', 'MOD')));

			DBMS_OUTPUT.PUT_LINE ('Updated ' || SQL%ROWCOUNT || ' records to ACT status');

			 /*  Update delivery pass status to ACT when the original order was a make good order/
				     entire order was discounted because customer had enough store credits.   */	
			UPDATE cust.delivery_pass SET status='ACT' WHERE status IN ('PEN') AND purchase_order_id IN (SELECT /*+ USE_NL(s, sa, sa2) */s.id FROM cust.sale s, cust.salesaction sa, cust.salesaction sa2 WHERE sa.requested_date >= TRUNC(SYSDATE - 10) AND sa.sale_id = s.id AND sa.action_type IN ('CRO', 'MOD') AND s.status = 'STL' AND sa.action_date = (SELECT MAX(action_date) FROM cust.salesaction WHERE sale_id = s.id AND action_type IN ('CRO', 'MOD')) AND s.id = sa2.sale_id AND sa2.action_type = 'INV' AND sa2.amount = 0 AND sa2.sub_total > 0);


			DBMS_OUTPUT.PUT_LINE ('Updated ' || SQL%ROWCOUNT || ' make good orders containing delivery pass to ACT status ');

			 /*  Update delivery pass status from EPG (Expired pending) to EXP when the last-placed orders got delivered.   */		
			UPDATE cust.delivery_pass SET status = 'EXP'  WHERE id NOT IN (SELECT dp.id  FROM cust.delivery_pass dp, cust.sale s WHERE s.dlv_pass_id IS NOT NULL AND dp.id = s.dlv_pass_id AND dp.status = 'EPG' AND s.status IN ('SUB','AUT','PRC','ENR','REF','NEW','MOD','MOC')) AND status = 'EPG';
		...
		ELSE
			...
			DBMS_OUTPUT.PUT_LINE ('Updated ' || SQL%ROWCOUNT || ' make good orders containing delivery pass to ACT status ');
			 ...
			 /*  Update delivery pass status from EPG (Expired pending) to EXP when the last-placed orders got delivered.   */		
			UPDATE cust.delivery_pass SET status = 'EXP'  WHERE id NOT IN (SELECT dp.id  FROM cust.delivery_pass dp, cust.sale s WHERE s.dlv_pass_id IS NOT NULL AND dp.id = s.dlv_pass_id AND dp.status = 'EPG' AND s.status IN ('SUB','PRC','ENR','REF','NEW','MOD','MOC')) AND status = 'EPG';
